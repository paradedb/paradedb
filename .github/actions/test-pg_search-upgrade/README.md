...
Keep `key_field` in these legacy `setup.sql` files: they run on released versions that require it, and `key_field_tokenizers` verifies compatibility with indexes written by those versions. Its explicit compatibility check in `queries.sql` also retains the option to verify that new indexes ignore it. Other current-version test fixtures omit the option.
