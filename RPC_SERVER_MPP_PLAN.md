...
- **Procedure**: A sub-plan fragment (subtree) that ends at a `DsmExchangeExec` node (acting as Producer).
- **RPC Server (Worker)**: Hosts a `StreamRegistry` of available Procedures. It sits idle until a request arrives.
- **RPC Client (`DsmExchangeExec` as Consumer)**: When executed, it sends a `StartStream(stream_id)` request to the node(s) hosting the required partitions.
...
- **`StreamSource`**: Redefine to store the actual input subtree (`Arc<dyn ExecutionPlan>`) rather than the `DsmExchangeExec` node.
- **`DsmExchangeExec`**:
  - **Unified Node**: Merges Producer (`DsmWriterExec`) and Consumer (`DsmReaderExec`) logic.
  - **Consumer Role**: `execute()` acts as the client. It sends `StartStream` and returns a stream reading from DSM.
  - **Producer Role**: Holds the `producer_partitioning` configuration used by `producer_task`.

### Step 2: Automatic Registration via Codec (`codec.rs`)

- **`try_decode`**:
  - When a `DsmExchangeExec` is deserialized, the worker will immediately extract its input child and producer configuration.
