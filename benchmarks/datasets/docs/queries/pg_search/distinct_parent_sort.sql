...
SET paradedb.enable_join_custom_scan TO off; EXPLAIN ANALYZE SELECT DISTINCT
    d.id,
    d.title,
    d.parents
FROM documents d
...
SET work_mem TO '64MB'; SET paradedb.enable_join_custom_scan TO on; EXPLAIN ANALYZE SELECT DISTINCT
    d.id,
    d.title,
    d.parents
FROM documents d
