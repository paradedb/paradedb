-- Shape: TopK Aggregate on a dense string key over a fan-out JOIN (DataFusion)
-- Join: stackoverflow_posts -> badges
-- Description: GROUP BY badges.name, a string every badge row carries and that
-- repeats across most of them, with COUNT(*) ordered DESC and LIMIT 10. This is
-- the Elasticsearch Terms Aggregation shape on a dense key: each matched post
-- fans out to all of its owner's badges, so the aggregate sees far more rows
-- than distinct names, and every row carries a real string, unlike the sparse
-- owner_display_name of join_aggregate_topk_count.
