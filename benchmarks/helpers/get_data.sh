...
db_query () {
  local QUERY=$1
  PGPASSWORD="$PASSWORD" docker exec paradedb psql -h "$HOST" -p "$PORT" -d "$DATABASE" -U "$USER" -c "$QUERY"
}
...
load_data () {
  ...
  db_query "CREATE TABLE IF NOT EXISTS temp_json ( j JSONB );"
  ...
  PGPASSWORD=$PASSWORD docker exec paradedb psql -h "$HOST" -p "$PORT" -d "$DATABASE" -U "$USER" -f helpers/load_data.sql
}
