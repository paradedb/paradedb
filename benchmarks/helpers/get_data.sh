...
db_query () {
  local QUERY=$1
  PGPASSWORD="$PASSWORD" docker exec -it paradedb psql -p "$PORT" -d "$DATABASE" -U "$USER" -c "$QUERY"
}
...
load_data () {
  ...
  PGPASSWORD=$PASSWORD ocker exec -it paradedb psql -p "$PORT" -d "$DATABASE" -U "$USER" -f helpers/load_data.sql
}
