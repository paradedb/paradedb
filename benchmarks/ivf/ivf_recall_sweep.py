...
fast field. Requires the candidate-gate engine (paradedb.vector_probe_gate_mode
+ the FRACTIONAL paradedb.vector_cluster_max_probe_fraction ceiling): the
...
def main():
    ...
    ap.add_argument("--probe-fractions", default="0.05",
                    help="comma-separated FRACTIONAL probe ceilings "
                         "(paradedb.vector_cluster_max_probe_fraction): each segment "
    ...
    ap.add_argument("--gate-modes", default="centroid,candidate",
                    help="comma-separated paradedb.vector_probe_gate_mode values to "
    ...
    if args.config:
        ...
        if "fanouts" in cfg_raw:
            raise SystemExit(
                ...
                "paradedb.vector_cluster_max_probe_fraction; set \"probe_fractions\".")
        if "probes" in cfg_raw:
            raise SystemExit(
                ...
                "paradedb.vector_cluster_max_probe_fraction. Replace with "
    ...
    with psycopg.connect(args.dsn, autocommit=True) as conn:
        ...
        try:
            conn.execute(f"SET paradedb.vector_probe_gate_mode = {gate_modes[0]}")
            conn.execute(
                f"SET paradedb.vector_cluster_max_probe_fraction = {fractions[0]}")
        except Exception as exc:
            raise SystemExit(
                f"[sweep] engine rejects the candidate-gate GUCs ({exc}); this sweep "
                f"needs the gate-mode engine (paradedb.vector_probe_gate_mode + "
        ...
        for filt in filters:
            ...
            conn.execute(
                f"SET paradedb.vector_cluster_max_probe_fraction = {warm_frac}")
            ...
            for mode in gate_modes:
                conn.execute(f"SET paradedb.vector_probe_gate_mode = {mode}")
                ...
                for k in ks:
                    ...
                    for eps in epsilons:
                        for frac in fractions:
                            conn.execute(
                                f"SET paradedb.vector_cluster_max_probe_fraction = {frac}")
                            conn.execute(
