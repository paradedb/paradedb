...
#
# FINGERPRINTS ARE REGIME-VERSIONED: a result-ID fingerprint taken under the
# work-unit budget (radii present; engine >= the work-unit rebase) does not
# compare to one taken under the legacy cluster-count budget — equal f buys
# nominal unique-candidate capacity under units vs replication-inflated
# candidate spend under counts, so probed sets differ by design. Reference
# fingerprint (unit regime, PER-INDEX open share x = ROWS_PER_OPEN/(
# ROWS_PER_OPEN + n̄), cohere-100k repl-2 rebuild, none/f=0.05/K10, 200q):
# sha256 4836b1ac… — re-recorded once when x moved from a fixed 0.07 to
# the per-index derivation (recall 0.9600 → 0.9605 at the reference
# cell); the fixed-x sha e9a87b2f… and the count-regime sha 9ce20f7a…
# are historical.
...
def explain_probe_stats(conn, sql, params):
    ...
    for seg, st in seg_info.items():
        ...
        merged["work_charged"] = merged.get("work_charged", 0.0) + st.get("work_charged", 0.0)
    ...
    extras = {
        ...
        # Routing observability: centroids the router actually scored and
        # its share of modeled work (routed x 1.45us vs the open/row terms
        # from the same fit) — the measured reason routing matters, and
        # deliberately NOT charged to the budget (see the cost-model notes
        # in the PR body).
        "router_scored": merged["router_scored"],
        # Observability: which segments fetched how many clusters. The
        # BUDGET identity is stated in units now, per segment:
        #   f·units_seg < work_charged_seg ≤ f·units_seg + last cluster's
        # charge (Ceiling terminations; radius-less segments meter counts).
        "probed_per_segment": ";".join(
            f"{seg}:{d['clusters_probed']}" for seg, d in sorted(segments.items())),
        "work_per_segment": ";".join(
            f"{seg}:{d.get('work_charged', 0.0):.4f}" for seg, d in sorted(segments.items())),
    }
...
def main():
    ...
    if per_query_rows is not None:
        pq_cols = ["rung", "filter", "probe_fraction", "k",
                   ...
                   "exhausted", "router_scored", "probed_per_segment", "work_per_segment",
