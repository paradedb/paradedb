...
(filter x probe-cap) grid. Requires the max_probes-era engine (tantivy pin
>= c4582807): the fanout GUC is gone; the cap is absolute clusters.
...
def main():
    ...
    ap.add_argument("--probes", default="65536,2048,256",
    ...
    probes = parse_int_list(args.probes)
        ...
        for filt in filters:
            ...
            for np_ in probes:
              for eps in epsilons:
                ...
                conn.execute(f"SET paradedb.vector_cluster_max_probes = {np_}")
