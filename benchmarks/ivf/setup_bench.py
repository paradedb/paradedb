...
def collect_build_stats(conn, table, index, *, load_secs, create_secs,
    ...
    COUNT SEMANTICS (max_probes-era engine): index_info.vector_num_vectors is
    ...
    sum(ivf_cluster_sizes) vs num_vectors, NOT num_vectors vs table rows
    ...
    row = conn.execute(
        ...
        "coalesce(sum(vector_num_vectors),0), coalesce(sum(vector_num_centroids),0), "
    ...
    (segments, total_b, post_b, pos_b, ff_b, fn_b, term_b, store_b,
     n_vectors, n_centroids, cmin, cmax, cavg, empties) = row
    ...
    # Memberships (rows incl. replicas) come from the per-cluster SRF; distinct
    # docs from index_info. Their ratio is the real replication factor.
    try:
        memberships = conn.execute(
            "SELECT coalesce(sum(size),0) FROM paradedb.ivf_cluster_sizes(%s)",
            (index,)).fetchone()[0]
        memberships = int(_f(memberships)) or None  # 0 => flat-only build
    except Exception:
        memberships = None  # SRF absent on this build
    try:
        pg_b = conn.execute(
    ...
    return {
        ...
        "structure": {
            ...
            "distinct_docs": n_vectors,          # index_info.vector_num_vectors
            "stored_entries": memberships,        # sum(ivf_cluster_sizes); None if SRF absent
...
def report_build_stats(stats, outdir, index):
    ...
    if st["stored_entries"] is None:
        rf_line = ("memberships unavailable (ivf_cluster_sizes SRF absent or "
...
def build_parser():
    ...
    ap.add_argument("--cluster-plot", action="store_true", default=_UNSET,
                    help="emit a cluster-size histogram + stats PNG after build (needs the "
                         "ivf_cluster_sizes SRF; default on)")
