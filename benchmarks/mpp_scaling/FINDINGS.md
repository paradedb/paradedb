...
**Update (after Path B lever 3 landed):** This interpretation was partially wrong. The real reason scalar count was flat under MPP wasn't that "CooperativeExec already saturates" — it was that **PG never actually launched workers for scalar count** even with `paradedb.enable_mpp=on` and `set_parallel(N)` requested, because `paradedb_upper_paths_callback` adds parallel-aware paths to the upper rel AFTER PG's `gather_grouping_paths` pass has already run, so no Gather consumer was ever generated for the partial path. The dummy 1e9 non-parallel path that `hook.rs::add_path` inserts as a fallback always won the cost comparison. So MPP was "marked active" but had `Workers Launched: 0`. After lever 3 explicitly wraps the partial path with a Gather via `pg_sys::create_gather_path`, scalar count jumps from flat to ~2× speedup at producers=4. CooperativeExec's segment-claim was real but it was running on **one** OS process; lever 3 unlocks the actual `parallel_workers=4` cross-process scan parallelism.
