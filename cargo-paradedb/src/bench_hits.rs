...
pub async fn bench_hits(url: &str, tag: &str, workload: &str, full_dataset: bool) -> Result<()> {
    ...
    if workload == "single" {
        ...
        run_cmd!(mkdir -p "/$HOME/partitioned")?;
        run_cmd!(
           seq 0 99
          | xargs "-P100" "-I{}" wget --no-verbose --directory-prefix "/$HOME/partitioned"
...
OPTIONS (path 'file:///$HOME/', extension 'parquet');
"#;
...
SELECT MIN(to_date("EventDate"::INT)), MAX(to_date("EventDate"::INT)) FROM hits;
...
SELECT "URL", COUNT(*) AS PageViews FROM hits WHERE "CounterID" = 62 AND to_date("EventDate"::INT) >= '2013-07-01' AND to_date("EventDate"::INT) <= '2013-07-31' AND "DontCountHits" = 0 AND "IsRefresh" = 0 AND "URL" <> '' GROUP BY "URL" ORDER BY PageViews DESC LIMIT 10;
SELECT "Title", COUNT(*) AS PageViews FROM hits WHERE "CounterID" = 62 AND to_date("EventDate"::INT) >= '2013-07-01' AND to_date("EventDate"::INT) <= '2013-07-31' AND "DontCountHits" = 0 AND "IsRefresh" = 0 AND "Title" <> '' GROUP BY "Title" ORDER BY PageViews DESC LIMIT 10;
SELECT "URL", COUNT(*) AS PageViews FROM hits WHERE "CounterID" = 62 AND to_date("EventDate"::INT) >= '2013-07-01' AND to_date("EventDate"::INT) <= '2013-07-31' AND "IsRefresh" = 0 AND "IsLink" <> 0 AND "IsDownload" = 0 GROUP BY "URL" ORDER BY PageViews DESC LIMIT 10 OFFSET 1000;
SELECT "TraficSourceID", "SearchEngineID", "AdvEngineID", CASE WHEN ("SearchEngineID" = 0 AND "AdvEngineID" = 0) THEN "Referer" ELSE '' END AS Src, "URL" AS Dst, COUNT(*) AS PageViews FROM hits WHERE "CounterID" = 62 AND to_date("EventDate"::INT) >= '2013-07-01' AND to_date("EventDate"::INT) <= '2013-07-31' AND "IsRefresh" = 0 GROUP BY "TraficSourceID", "SearchEngineID", "AdvEngineID", Src, Dst ORDER BY PageViews DESC LIMIT 10 OFFSET 1000;
