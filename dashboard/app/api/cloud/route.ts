...
export async function GET(req) {
    try {
        ...
        if (!provisionerResponse.ok) {
            ...
            return NextResponse.json({ status: 'error', message: errorMessage }, { status: provisionerResponse.status });
        }
        ...
        return NextResponse.json({ status: 'error', message: 'Failed to connect to the server.' }, { status: 500 }); // 500 for Internal Server Error
    }
}
