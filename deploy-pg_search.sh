...
if [ "$VERSION" == "unsert" ]; then
  ...
  read -r VERSION
  if [ -z "$VERSION" ]; then
    echo "Using default and installing latest pg_search"
    VERSION="latest"
  else
...
if [ "$OS" == "Linux" ]; then
  ...
  echo ""
  sudo apt-get install -y libicu70 curl
elif [ "$OS" == "Darwin" ]; then
  brew install icu4c curl
else
...
curl -L "https://github.com/paradedb/paradedb/releases/download/$VERSION/pg_search-v$VERSION-pg${version}-$ARCH-ubuntu2204.deb" -o /tmp/pg_search.deb 
sudo apt-get install -y /tmp/pg_search.deb

echo "By default, the pg_search extension is installed with anonymous telemetry enabled."
...
if [ "$EMAIL" == "unsert" ]; then
  echo "To help us better understand how you use pg_search, we would like to collect your email address."
...
echo "CREATE EXTENSION pg_search;"
echo ""
echo "To get started with pg_search, check out the docs: https://docs.paradedb.com/search/quickstart"
