...
-- Replace the dummy schema with the actual schema
CREATE FOREIGN TABLE csv_table (x INT)
SERVER csv_server
...
The `sniff_csv` function returns the detected CSV properties as a table with a single row. An optional
`sample_size` argument configures the number of rows sampled.

```sql
SELECT * FROM sniff_csv('my_file.csv');
SELECT * FROM sniff_csv('my_file.csv', sample_size => 1000);
```
