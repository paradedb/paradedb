...
Parquet tables are written to the write-ahead logs (WAL) on Postgres 15+. This is because WAL support is
enabled by the custom WAL manager API, which was introduced in Postgres 15.
