...
<Note>
`value` is treated as a token. [Additional processing](/api-reference/autocomplete#fuzzy-phrase) is required to perform fuzzy search
over a phrase containing multiple tokens.
</Note>
...
<ParamField body="value" required>
  Defines the term you are searching for within the specified field,
