...
<Note>
  BM25 scoring is not enabled for TermSet queries. As a result, the scores
...
Matches documents containing any [term](/api-reference/concepts/term) from a specified set.
