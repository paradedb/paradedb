...
passing the entire phrase to `fuzzy_term` will not yield any matches. Instead, `fuzzy_phrase` should be used. `fuzzy_phrase` tokenizes the query and
matches documents if any of the tokens match.
...
`fuzzy_phrase` can be configured to match documents if all tokens match, which can narrow the result set.
