...
To facilitate directly querying a table using text search queries ParadeDB provides a custom operator, named `@@@`. It has two
forms:
...
Using the `@@@` operator means "find all rows in the table that match a text search query". It's designed to answer
such questions as quickly as possible and allow complex SQL queries that also include normal SQL query predicates.

The `@@@` operator interacts well with `ORDER BY`, `OFFSET/LIMIT`, `GROUP BY` clauses as well as standard query operators
such as `AND`, `OR`, and `NOT`.

Unlike the `idxname.search()` and `idxname.score_bm25()` search functions, the `@@@` **does not** itself perform any
scoring, ordering, offset or limit. These are now operations that need to be added to the overall query.
...
```sql
SELECT * FROM paradedb.bm25_search WHERE description ilike '%shoes%';
```

<Accordian title="Expected Response">
```csv
