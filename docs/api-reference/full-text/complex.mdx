...
The `search` function that's generated for your index can accept query objects instead of a query string. Nearly all the query types implemented in Tantivy, our
underlying search engine, are exposed to Postgres. If needed, you can directly refer to the [Tantivy documentation](https://docs.rs/tantivy/latest/tantivy/) as well.
