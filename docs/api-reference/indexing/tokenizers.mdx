...
<ParamField body="stemmer">
  Applies language-specific stemming to tokens. Available stemmers are `Arabic`, `Danish`, `Dutch`, `English`, `Finnish`, `French`, `German`, `Greek`, `Hungarian`, `Italian`, `Norwegian`, `Portuguese`, `Romanian`, `Russian`, `Spanish`, `Swedish`, `Tamil`, and `Turkish`.
</ParamField>

## Inspecting a Tokenizer

`paradedb.tokenize` tokenizes the input text with the specified tokenizer. It's useful for comparing different tokenizers.
