...
Treats the entire text as a single token. Filters out tokens larger than 255 bytes.
...
At search time, queries are automatically tokenized in the same way as the target field.

To manually tokenizes input text with a specified tokenizer, use `paradedb.tokenize`. This function is useful for comparing different tokenizers or
...
In ParadeDB, only one tokenizer is allowed per field and only one BM25 index is allowed per table. However, some advanced search cases may require the same field to be tokenized in multiple ways. For instance, tokenizing on both whitespace and
ngrams may improve how search results are ranked.

One way to achieve this is to use generated columns. For instance, suppose we 