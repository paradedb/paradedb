...
- Return hundreds of thousands or millions of documents as quickly as possible
...
<Note>
We are undergoing an internal refactor to move more of the API way from the `<index_name>.search` syntax and towards the `@@@` operator, which delivers a number of performance and
syntax improvements. Over time, you can expect the `@@@` operator to be recommended more heavily across the documentation.
</Note>
