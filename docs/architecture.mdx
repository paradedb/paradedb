...
Retake employs logical-replication-based Change-Data-Capture (CDC) to pipe data from Postgres to the search layer. By default, Retake implements change data capture (CDC)
over Postgres to stay in sync — when a row is changed or new data arrives in Postgres, Retake automatically updates
...
Neural search is a technique for delivering more accurate, contextually-aware search results by merging traditional
keyword-based search algorithms like BM25 with semantic search.
