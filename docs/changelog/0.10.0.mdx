...
The [documentation](/api-reference/optimization/overview) covers its usage, but it's worth mentioning that using the `@@@` operator in a WHERE clause, as opposed to SELECTing directly from `idxname.search(...)`, can lean to drastically improved query execution times in situations where scores (or the effects of scoring on ordering) are not necessary.
