...
- This release introduces efficient handling of aggregate queries (e.g. `COUNT(<field>)`, `SUM(<field>)`, etc.), where `<field>` is indexed as a fast field. This [optimization](TODO) is automatically applied by the query planner when these queries are executed.
