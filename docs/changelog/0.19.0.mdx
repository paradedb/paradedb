...
canonical: https://docs.paradedb.com/changelog/0.19.0
---
...
- A new way to specify index configuration options like [tokenizers](/v2/tokenizers/overview) and [token filters](/v2/token-filters/overview) without the
  old JSON string syntax
- First class support for [multiple tokenizers per field](/v2/tokenizers/multiple-per-field)
- New token filters like [alpha numeric only](/v2/token-filters/alphanumeric) and [token length](/v2/token-filters/token-length)
- A new way to cast queries to [fuzzy](/v2/full-text/fuzzy), [slop](/v2/full-text/phrase#adding-slop), and [boost](/v2/sorting/boost)
- An improved [more like this](/v2/query-builder/specialized/more-like-this) function
