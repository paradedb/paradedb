...
<Note>
  `0.20.0` promotes the v2 API to default.
   The v2 API should be used for all new indexes. The original API is now referred to as the legacy API and will be removed in a future version.
</Note>

<Note>
   While there are no breaking changes, the default tokenizer has been changed from `simple` to `unicode_words`, which will affect new indexes that 
   don't specifiy tokenizers for all columns.
</Note>
...
ParadeDB 0.20 introduces powerful [search aggregation capabilities](/documentation/aggregates) through the new `pdb.agg()` function. This function can be used in two ways to push analytics down into the Tantivy index for optimal performance.
...
It has now been promoted to the default API and can be found documented at [http://docs.paradedb.com/documentation](/documentation/). The old API can be found at [https://docs.paradedb.com/legacy](/legacy) and will be removed in a future version.
...
- Columnar fast fields by default for all non-text and literal tokenized types, removing the need to confugure manually.
...
**Improved Default Tokenizer**: The default [tokenizer](/documentation/indexing/tokenizers) has been changed to `pdb.unicode_words`, which splits text based on the [Unicode Standard Annex #29](http://www.unicode.org/reports/tr29/) rules for better international text support.
...
**JSON Key Sorting**: Added fast [sorting](/documentation/full-text/sorting) support for JSON keys, allowing efficient ordering by nested JSON field values.
