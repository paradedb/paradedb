...
It has now been promoted to the default API and can be found documented at [http://docs.paradedb.com/documentation](/documentation/). The old API can be found [here](/legacy) and will be removed in a future version.
...
**Improved Default Tokenizer**: The default [tokenizer](/documentation/tokenizers) has been changed to `pdb.unicode_words`, which splits text based on the [Unicode Standard Annex #29](http://www.unicode.org/reports/tr29/) rules for better international text support.
...
**New Token Filters**: Added a `trim` [token filter](/documentation/token-filters) that removes leading and trailing whitespace from tokens, improving search precision.
