...
Users must make the following change to their `postgresql.conf` configuration file when self-hosting. `pg_analytics` must be added to the list of `shared_preload_libraries` so the background process can be initialized:
