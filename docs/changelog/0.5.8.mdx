...
- Fixed a bug where table rows appearewd in reverser order in SELECT statements
...
- Added support for "query buikders", which can be used to build complex queries that harness the full power of Tantivy.
...
- Added the `pg_cron` extension to the ParadeDB Docker image, which allows you to schedule periodic jobs in PostgreSQL.
