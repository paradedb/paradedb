...
canonical: https://www.paradedb.com/docs/concepts/architecture
---

import { ArchitectureDiagram } from "/snippets/architecture-details.jsx";
...
Works](/concepts/how-the-paradedb-index-works).
...
row-oriented layout. Non-text, non-JSON fields are stored in columnar storage by default. Text and JSON fields can opt into columnar storage through tokenizer configuration, and the [literal](/reference/tokenizers/available-tokenizers/literal) and [keyword](/reference/tokenizers/available-tokenizers/keyword) tokenizers are columnar by default.

### Vector Index

When a `vector` column is indexed with a vector operator class, ParadeDB stores it in ParadeDB-built vector index structures alongside text and columnar fields. This lets vector retrieval compose with full-text predicates, filters, ranking, and aggregations inside the same ParadeDB index. See [Indexing Vectors](/reference/indexing/indexing-vectors) for setup details.
...
An LSM tree is a write-optimized data structure commonly used in systems like RocksDB and Cassandra. The core idea behind an LSM tree is to turn random writes into sequential ones. Incoming writes are appended to a mutable segment, which buffers rows across statements. Once the segment reaches the configured [`mutable_segment_rows`](/operate/performance-tuning/writes#increase-mutable-segment-size) threshold—1,000 rows by default—it is frozen and becomes eligible for conversion into an immutable segment.
...
ParadeDB introduces several new text search operators to Postgres. For example, `|||` is used for [match disjunction](/reference/full-text/match) queries, whereas `###`
is for [phrase](/reference/full-text/phrase) queries.
...
allow the extension to "take over" large parts of the query, including aggregates, `WHERE`, and even [`GROUP BY` clauses](/project/roadmap#deeper-analytics-improvements).
...
For queries that need to read large amounts of data, such as [Top K](/reference/full-text/top-k) or aggregate queries, ParadeDB custom scans can use additional Postgres workers to execute the query
...
<Note>
  ...
  workers](/operate/performance-tuning/reads).
</Note>
