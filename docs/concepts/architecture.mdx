...
row-oriented layout. All text fields which use the [literal](/reference/tokenizers/available-tokenizers/literal) or [literal normalized](/reference/tokenizers/available-tokenizers/literal-normalized) tokenizer, or are non-text,
...
allow the extension to "take over" large parts of the query, including aggregates, `WHERE`, and even [`GROUP BY` clauses](/project/roadmap#analytics).
