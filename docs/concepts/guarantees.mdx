...
Under `SERIALIZABLE`, a search read takes a predicate lock on the whole table it reads, the way a sequential scan does. A search predicate covers no key range, and the scan can answer a query from the index without reading the rows it matched, so there is nothing finer to lock. Any concurrent write to the table conflicts with the read, including a write to a row the search did not match.
...
By default, ParadeDB favors correctness, even when it comes at the cost of slower query execution. For `pdb.agg`, transaction visibility filtering is enabled by default. If your workload can tolerate potentially stale aggregate results, you can skip it with `visibility => 'raw'` to improve performance, or let the query's size decide with `visibility => 'threshold'`. Both settings answer from the index alone, so under `SERIALIZABLE` the read cannot conflict with a concurrent write that its snapshot does not see.
