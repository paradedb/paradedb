...
Under `SERIALIZABLE`, a search read takes a predicate lock on the whole table it reads, the way a sequential scan does. A search predicate covers no key range, and a scan can answer from the index without reading the rows it matched. A write from another `SERIALIZABLE` transaction then creates a read/write dependency with the read, including a write to a row the search did not match. One dependency is not an abort: Postgres cancels a transaction only when the dependencies it collects have no serial order.
...
By default, ParadeDB favors correctness, even when it comes at the cost of slower query execution. For `pdb.agg`, transaction visibility filtering is enabled by default. If your workload can tolerate potentially stale aggregate results, you can skip it with `visibility => 'raw'` to improve performance, or let the query's size decide with `visibility => 'threshold'`. A read that skips the heap records no dependency on a write that committed before it and that its snapshot cannot see. That holds for `'raw'` always, and for `'threshold'` above the estimate.
...
If your deployment requires high availability, failover, or read replicas for ParadeDB queries, use [ParadeDB Enterprise](/operate/deploy/enterprise).
