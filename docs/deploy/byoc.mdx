...
ParadeDB BYOC is built on [CloudNativePG](https://cloudnative-pg.io/), the leading Kubernetes operator for Postgres, and is typically deployed alongside an existing OLTP Postgres-as-a-Service platform such as AWS RDS. It
includes a primary-replica cluster with built-in logical replication ingestion and automated backups.
