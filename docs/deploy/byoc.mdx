...
ParadeDB BYOC (Bring Your Own Cloud) is a managed deployment of ParadeDB within your cloud environment. It enables tight integration with managed Postgres services
like AWS RDS and combines the benefits of a managed platform with the security posture of a self-hosted deployment.

ParadeDB BYOC is supported on AWS, including GovCloud regions, and GCP. It is also supported in airgapped environments. To request access for Azure, Oracle Cloud, or
another cloud platform please contact [sales@paradedb.com](mailto:sales@paradedb.com).
