    ...
    steps:
      ...
      - name: Run ParadeDB example queries
        run: |
          ...
          psql -h localhost -U testuser -d testdb -c "CREATE INDEX search_idx ON mock_items USING paradedb (id, description, category, rating, in_stock, created_at, metadata, weight_range);"
