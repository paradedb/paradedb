...
Key indicators of successful integration:

- `Custom Scan (Citus Adaptive)` shows Citus is distributing the query
- `Custom Scan (ParadeDB Scan)` shows BM25 index is being used on each shard
- `Task Count: 32` shows parallel execution across shards
