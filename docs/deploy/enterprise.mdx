...
| Maximum cluster size                 | Unlimited          | Unlimited           |
...
| ParadeDB BYOC                        | ❌                 | ✅                  |
| Commercial license / AGPL waiver     | ❌                 | ✅                  |
| Commercial support                   | ❌                 | ✅                  |
...
<p className="text-sm">
  ...
  I/O.
  3. All listed deployment features and limitations are specific to the BM25
  index. Physical replication and recovery assume every node in the cluster is
  running a compatible PostgreSQL and `pg_search` build.
</p>
</Info>
