...
`pg_search` and `pg_analytics`. In order to keep the size of the ParadeDB image small, only the ParadeDB extensions and a limited set of necessary
third party extensions are included in the image.

It is, however, possible to install arbitrary third party extensions in the ParadeDB image.
...
Users do not need to install these extensions manually nor run `CREATE EXTENSION` manually. They will be available by default in any
...
- Creating the extension in a Postgres database inside the ParadeDB image to activate it

We recommend installing prebuilt binaries from APT whenever available to keep the image size as small as possible. Most popular
Postgres extensions maintain binaries available via `apt-get install`.

The example below will showcase installing PostGIS, a popular extension for geospatial data in Postgres, in the ParadeDB image.

<Accordion title="Installing Third Party Extension Example - PostGIS">
...
- [pgrepack - Reorganize Tables in PostgreSQL Databases with Minimal Locks](https://github.com/reorg/pg_repack)
