...
While GCP offers a wide range of OS images, choose one where you can install Docker Engine, such as the Debian or Ubuntu images.

Once you've made the right choices, launch the instance either via the GCP Console's "Create Instance" button or use the gcloud SDK.

**Networking**

Configure firewall rules on the GCP console or CLI to allow TCP traffic to the port where ParadeDB will be exposed.
...
Update necessary packages and add the docker repository:
...
<Note>It's recommended to log out and log back in for group changes to take effect.</Note>

**Step 3: Run the ParadeDB image**

Pull and start the ParadeDB Docker image:
...
Ensure that your instance's firewall rules allow connections on the port where PostgreSQL is operating.