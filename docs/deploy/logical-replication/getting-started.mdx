...
Deploy ParadeDB on the subscriber, for example via [Docker](/documentation/getting-started/install),
[Kubernetes](/deploy/self-hosted/kubernetes), or by [installing the extension](/deploy/self-hosted/extension)
into an existing Postgres. Then load the extension in the subscriber database:
...
Guide](/deploy/logical-replication/operational-guide) for ParadeDB
