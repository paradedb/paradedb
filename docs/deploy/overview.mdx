...
For hobby environments, [ParadeDB Community](https://github.com/paradedb/paradedb) can be deployed as a [Docker image](/deploy/self-hosted/docker),
...
<Note>
Running ParadeDB Community in a production environment is discouraged.

This is because ParadeDB Community [does not have write-ahead log (WAL) support](/deploy/enterprise). Without WALs, the index is not guaranteed to be durable. This means that data can be lost or the
entire index can become corrupt if the server crashes or restarts, which is common during failovers or upgrades. For more details, see [guarantees](/welcome/guarantees#aci-d).
