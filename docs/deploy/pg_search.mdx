...
ParadeDB provides prebuilt binaries for the `pg_search` extension for Postgres 14, 15 and 16 on Ubuntu 22.04. They can be found in [GitHub Releases](https://github.com/paradedb/paradedb/releases/latest). If you
...
```bash
curl -L "https://github.com/paradedb/paradedb/releases/download/v0.7.0/pg_search-v0.7.0-pg16-arm64-ubuntu2204.deb" -o /tmp/pg_search.deb
sudo apt-get install -y /tmp/*.deb
```

Note: You can replace `v0.7.0` with the `pg_search` version you wish to install, and `pg16` with the version of Postgres you are using.

## Building from Source

Please follow these [instructions](https://github.com/paradedb/paradedb/tree/v0.6.1/pg_search#installation).
