...
For production deployments, we recommend running ParadeDB on Kubernetes with [CloudNativePG](https://cloudnative-pg.io/). CloudNativePG manages the Postgres cluster, including high availability and read replicas. Both ParadeDB Community and Enterprise binaries can be deployed on Kubernetes.

<Note>
  We recommend running ParadeDB Enterprise, not Community, with Kubernetes in
  production to maximize uptime. See
  [overview](/deploy/overview#self-hosted-paradedb).
</Note>
