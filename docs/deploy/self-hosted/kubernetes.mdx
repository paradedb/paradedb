...
For production deployments, we recommend running ParadeDB on Kubernetes with [CloudNativePG](https://cloudnative-pg.io/). CloudNativePG manages the Postgres cluster, including high availability and read replicas. Both ParadeDB Community and Enterprise can be deployed this way.
