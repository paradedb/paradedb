...
The latest version of `pg_search` is `0.21.9`. Please refer to the [releases](https://github.com/paradedb/paradedb/releases) page for all available versions of `pg_search`.
...
```bash
docker pull paradedb/paradedb:0.21.9
```

The latest version of the Docker image should be `0.21.9`.
...
```sql
ALTER EXTENSION pg_search UPDATE TO '0.21.9';
```
