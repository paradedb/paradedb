...
1. [`pg_search`](https://github.com/paradedb/paradedb/releases) is on version `0.10.1`
...
The latest version of the Docker image should be `0.10.1`. Then, run the following commands to upgrade all extensions to their latest version.

```sql
ALTER EXTENSION pg_search UPDATE TO '0.10.1';
