...
1. [`pg_search`](https://github.com/paradedb/paradedb/releases) is on version `0.10.2`
2. [`pg_analytics`](https://github.com/paradedb/pg_analytics/blob/main/pg_analytics.control) is on version `0.2.0`
...
The latest version of the Docker image should be `0.10.2`. Then, run the following commands to upgrade all extensions to their latest version.

```sql
ALTER EXTENSION pg_search UPDATE TO '0.10.2';
ALTER EXTENSION pg_analytics UPDATE TO '0.2.0';
