...
ParadeDB ships all of its functionality via a Postgres extension. All updates to ParadeDB can be
received by updating the `pg_search` extension.
...
Because `pg_search` and `pgvector` are independent extensions, they each have their own versions.
For the latest available version, please refer to the respective Github repos:

1. [`pg_search`](https://github.com/paradedb/paradedb/releases) is on version `0.15.8`
2. [`pgvector`](https://github.com/pgvector/pgvector/tags) is on version `0.8.0`
...
The latest version of the Docker image should be `0.15.8`.
...
```sql
<<<<<<< HEAD
ALTER EXTENSION pg_search UPDATE TO '0.15.8';
ALTER EXTENSION pg_analytics UPDATE TO '0.3.5';
=======
ALTER EXTENSION pg_search UPDATE TO '0.15.7';
>>>>>>> ed4de839 (Remove pg_analytics and pg_parquet)
