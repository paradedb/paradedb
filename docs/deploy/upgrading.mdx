...
2. [`pg_analytics`](https://github.com/paradedb/pg_analytics/blob/main/pg_analytics.control) is on version `0.2.2`
...
ALTER EXTENSION pg_analytics UPDATE TO '0.2.2';
...
2. Download and install the extension(s) you wish to upgrade in the same way that it was initially insalled (e.g. via cURL)
3. Start Postgres
4. Run `ALTER EXTENSION` in every database that has previously run `CREATE EXTENSION <extension_name>`
