...
<Tip>
  If the source and sink servers are running as Docker containers in the same machine, you can connect them to the Docker
  Compose network so they are accessible by the realtime server. This can be done with the `docker network connect <network> <container>`
