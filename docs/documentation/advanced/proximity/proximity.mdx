...
A proximity query consists of a [`ProximityClause`](/documentation/advanced/proximity/proximity_clause), a distance value, and another `ProximityClause`. As mentioned, a `ProximityClause` can be a term or a regular expression. It can also be another `ProximityClause` or an array of them.
...
<CodeGroup>
```sql Advanced Example
SELECT * FROM mock_items WHERE id @@@ paradedb.proximity(
    'description', -- field to query
    paradedb.prox_array('generic', 'white', paradedb.prox_regex('sl..k')), -- left
    1, -- word/token distance
    'shoes' -- right
);
```
</CodeGroup>
