...
WHERE id @@@ paradedb.range_term('weight_range', 3);
```
...
WHERE id @@@ paradedb.boolean(
    must => ARRAY[
        paradedb.range_term('weight_range', 3),
...
```sql
SELECT id, weight_range FROM mock_items
WHERE id @@@ paradedb.range_term('weight_range', '(5, 15]'::int4range, 'Intersects');
```
...
```sql
SELECT id, weight_range FROM mock_items
WHERE id @@@ paradedb.range_term('weight_range', '(2, 11]'::int4range, 'Contains');
```
...
```sql
SELECT id, weight_range FROM mock_items
WHERE id @@@ paradedb.range_term('weight_range', '(2, 5]'::int4range, 'Within');
```
