...
Matches documents containing a specified [term](/documentation/concepts/term). Term queries are used when you need to match values exactly as they are stored in the index, without any analysis or tokenization.
Example:
...
Avoid using term query for text field as term query does not analyze the search term. The term query looks for the exact term you provide and hence this may return poor or no results.
