...
Multiple predicates should be combined into a single `paradedb.boolean` instead of using Postgres `AND`, `OR`, or `NOT`. This is a temporary limitation we expect
