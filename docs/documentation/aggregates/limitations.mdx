...
A few `pdb.agg()` shapes run on a single table but not over a join: metric aggregations over JSON sub-fields, such as `{"sum": {"field": "metadata.qty"}}`, `terms` on an array field or on a JSON sub-field that holds numbers, and `min_doc_count: 0`, which lists every term of the column. Among buckets with equal counts the order is not defined, so a `size` cutoff between them can keep any of them.
