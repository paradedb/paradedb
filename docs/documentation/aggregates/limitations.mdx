...
| `pdb.agg()` | `terms` (with `size`, `min_doc_count` of 1 or more, `missing`, and `order` by `_count`, `_key`, or a metric sub-aggregation), `sum`, `avg`, `min`, `max`, `value_count`, `cardinality`, and nested `aggs` built from these. The `visibility` argument is honored; over a join, `threshold` makes one decision for every table, judged on the largest table's estimate. |

A `pdb.agg()` field name must belong to exactly one indexed table in the join. When the same field name exists in several tables, qualify it with the table alias, as in `"field": "p.category"`.

A few details differ from the single-table Tantivy backend. Metric aggregations over JSON sub-fields, such as `{"sum": {"field": "metadata.qty"}}`, are not supported over joins; `terms` on a text-valued JSON sub-field is. `min_doc_count: 0`, which on Tantivy lists every term of the column, is not supported over joins. Buckets with equal counts sort by key here, while Tantivy leaves them in collection order, so a `size` cutoff can keep different buckets on the two backends.
