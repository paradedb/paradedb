...
| `pdb.agg()` | `terms` (with `size`, `min_doc_count`, `missing`, and `order` by `_count`, `_key`, or a metric sub-aggregation), `sum`, `avg`, `min`, `max`, `value_count`, `cardinality`, and nested `aggs` built from these. The `visibility` argument is honored. |
...
Two details differ from the single-table Tantivy backend. Metric aggregations over JSON sub-fields, such as `{"sum": {"field": "metadata.qty"}}`, are not supported over joins; `terms` on a JSON sub-field is. Buckets with equal counts sort by key here, while Tantivy leaves them in collection order, so a `size` cutoff can keep different buckets on the two backends.
