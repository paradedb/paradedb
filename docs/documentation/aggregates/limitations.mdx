...
ParadeDB uses its DataFusion backend for this conversion, preserving PostgreSQL's date boundaries across the full finite timestamp range, as well as `infinity`, `-infinity`, and the `NULL` group. Date grouping can be combined with other grouping columns and per-aggregate `FILTER` clauses.
