...
ActiveRecord::Base.connection.exec_query(<<~SQL)
  SELECT pdb.agg('{"top_hits": {"size": 3, "sort": [{"created_at": "desc"}], "docvalue_fields": ["id", "created_at"]}}') AS agg
  FROM mock_items
  WHERE id @@@ pdb.all()
  GROUP BY rating;
SQL
...
# coming soon
ActiveRecord::Base.connection.exec_query(<<~SQL)
  SELECT pdb.agg('{"top_hits": {"size": 3, "from": 1, "sort": [{"created_at": "desc"}], "docvalue_fields": ["id", "created_at"]}}') AS agg
  FROM mock_items
  WHERE id @@@ pdb.all()
  GROUP BY rating;
SQL
...
# coming soon
ActiveRecord::Base.connection.exec_query(<<~SQL)
  SELECT pdb.agg('{"top_hits": {"size": 3, "sort": [{"created_at": "desc"}, {"id": "asc"}], "docvalue_fields": ["id", "created_at"]}}') AS agg
  FROM mock_items
  WHERE id @@@ pdb.all()
  GROUP BY rating;
SQL
