...
ParadeDB supports aggregates through plain SQL functions like `COUNT`, `SUM`, and `AVG`. For instance, the following query finds the total number of search results for a full text query.
...
The performance of aggregates over very large result sets (i.e. millions of rows) is a well-known problem in Postgres.
...
In order for a query to be accelerated, **all** of the returned columns must be fast fields. For instance, the following query is accelerated because both `rating`
and `id` are fast fields:

```sql
SELECT rating, COUNT(id) FROM mock_items
WHERE description @@@ 'shoes'
...
```sql
EXPLAIN SELECT rating, COUNT(id) FROM mock_items
WHERE description @@@ 'shoes'
GROUP BY rating LIMIT 5;
```
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=11.53..11.57 rows=2 width=12)
   ->  GroupAggregate  (cost=11.53..11.57 rows=2 width=12)
         Group Key: rating
         ->  Sort  (cost=11.53..11.54 rows=2 width=8)
               Sort Key: rating
               ->  Custom Scan (ParadeDB Scan) on mock_items  (cost=10.00..11.52 rows=2 width=8)
                     Table: mock_items
                     Index: search_idx
                     Fast Fields: rating, id
                     Scores: false
...
```sql
SELECT description, COUNT(id) FROM mock_items
WHERE description @@@ 'shoes'
...
```sql
EXPLAIN SELECT description, COUNT(id) FROM mock_items
WHERE description @@@ 'shoes'
GROUP BY description LIMIT 5;
```
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=14.04..14.08 rows=2 width=40)
   ->  GroupAggregate  (cost=14.04..14.08 rows=2 width=40)
         ...
         ->  Sort  (cost=14.04..14.04 rows=2 width=36)
               Sort Key: description
               ->  Index Scan using search_idx on mock_items  (cost=10.00..14.03 rows=2 width=36)
...
Over time, a table's [visibility map](https://www.postgresql.org/docs/current/storage-vm.html) can become outdated. This can negatively impact aggregate performance by increasing the time it takes to perform visiblity checks over individual rows.
