...
Every `INSERT`/`UPDATE`/`COPY` statement creates at least one new segment. Generally speaking, search performance is improved if the number of segments is equal to the number of [parallel workers](/documentation/configuration/read-throughput).
...
The merge policy is a configurable [layered](/documentation/configuration/write-throughput) policy that attempts to merge segments into
