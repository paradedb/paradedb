...
`paradedb.segment_merge_scale_factor` is an integer, defaulting to `5`, that can be used to defer merging segments util the index has at least `"available parallelism" * paradedb.segment_merge_scale_factor + 1` segments.
The larger the value, the more segments will be merged together at once, which can improve write performance at the cost of search performance.
