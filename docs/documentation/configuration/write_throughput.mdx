...
The `paradedb.global_mutable_segment_rows` setting enables use of mutable segments, which buffer new rows in order to amortize the cost of indexing them.
...
A higher `paradedb.global_mutable_segment_rows` generally improves write throughput at the expense of read performance,
