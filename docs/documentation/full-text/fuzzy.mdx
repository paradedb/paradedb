...
# coming soon: fuzzy match disjunction/conjunction wrappers
```
...
```ruby Rails
MockItem.search(:description)
        .fuzzy("rann", distance: 1, prefix: true)
        .select(:id, :description)
        .limit(5)
```
...
```ruby Rails
MockItem.search(:description)
        .fuzzy("slee", distance: 1, prefix: true)
        .select(:id, :description)
        .limit(5)

# coming soon: multi-term fuzzy conjunction wrapper
```
...
```ruby Rails
MockItem.search(:description)
        .fuzzy("shose", distance: 1, prefix: false)
        .select(:id, :description)
        .limit(5)

# coming soon: transposition_cost_one toggle is not exposed directly
```
