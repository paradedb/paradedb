...
MockItem.objects.filter(
    description=ParadeDB(Fuzzy('runing', distance=2), Fuzzy('shose', distance=2))
).values('id', 'description')[:5]

# Fuzzy match conjunction is not currently exposed by django-paradedb.
# Fuzzy term (=== ...::pdb.fuzzy(...)) is not currently exposed by django-paradedb.
```
...
```python Django
# Fuzzy prefix options are not currently exposed by django-paradedb.
```
...
```python Django
# Fuzzy transposition options are not currently exposed by django-paradedb.
```
