...
```python Django
# This SQL block contains two statements (`|||` and `&&&`), but django-paradedb
# does not expose an exact `||| 'shoes'` form for a single-token literal.
```
...
```python Django
# django-paradedb does not currently expose a custom query tokenizer override.
```
