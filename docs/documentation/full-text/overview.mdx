...
`@@@` is ParadeDB's full text search operator that means "find all rows matching a full text query." The left-hand side of `@@@` accepts the field to search against, and the right-hand side accepts
a query string.
