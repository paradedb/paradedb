...
```ruby Rails
MockItem.where("description ### 'running shoes'::pdb.whitespace")
        .select(:description, :rating, :category)
```
...
```ruby Rails
MockItem.where("description ### ARRAY['running', 'shoes']")
        .select(:description, :rating, :category)
```
...
```ruby Rails
MockItem.where("description ### ARRAY['shoes', 'running']::pdb.slop(2)")
        .select(:description, :rating, :category)
```
