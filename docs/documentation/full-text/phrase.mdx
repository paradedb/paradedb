...
```python Django
MockItem.objects.bulk_create([
    MockItem(description='running sleek shoes', rating=5, category='Footwear'),
    MockItem(description='shoes running', rating=5, category='Footwear'),
])
```

```ruby Rails
MockItem.insert_all([
  { description: "running sleek shoes", rating: 5, category: "Footwear" },
  { description: "shoes running", rating: 5, category: "Footwear" }
])
```
