...
Update your development database config in `config/database.yml`:
...
Generate a migration to create the `mock_items`:
...
Now create a model for the `mock_items` table in `app/models/mock_item.rb`:
...
And a search index for that table
...
And run some queries in your console:
