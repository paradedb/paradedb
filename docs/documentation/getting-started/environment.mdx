...
To start you'll need to install SQLAlchemy, Alembic, [psycopg](https://www.psycopg.org/) and [django-paradedb](https://pypi.org/project/django-paradedb/).
...
and update the Alembic configuration to point to your database:

```ini alembic.ini
sqlalchemy.url = postgresql+psycopg://myuser:mypassword@localhost:5432/mydatabase
```

Copy this configuration into your `env.py`:
...
# This import is required for autogenerated ParadeDB migrations to work properly
...
# The ParadeDB Docker image comes pre-bundled with some popular extensions like PostGIS. PostGIS automatically creates a table called `spatial_ref_sys`. This tells Alembic to ignore it so that it doesn't try to drop it even though it's not tracked in Alembic's metadata.
...
We can now create a migration for ParadeDB's built-in test table. First, create a blank migration in `0001_create_mock_items_table.py` by running the following command:

```bash
alembic revision --rev-id 0001 -m "Create mock_items table"
```

Update the generated migration to create the test table
...
Now, create a file called `models.py` with a model and search index for `mock_items`:
...
We can now autogenerate a new migration to create the search index.

```bash
alembic revision --rev-id 0002 --autogenerate -m "Create search index on mock_items"
```

The generated migration in should look like this
...
Open up a Python shell and paste in the following:
