...
We can now add a model for ParadeDB's built-in test table and ParadeDB index:
...
def upgrade() -> None:
    ...
    op.create_paradedb_index('search_idx', 'mock_items', ['id', 'description', 'embedding vector_cosine_ops', 'category', 'rating', 'in_stock', 'metadata', 'created_at', 'last_updated_date', 'latest_available_time', 'weight_range'], table_schema='public')
