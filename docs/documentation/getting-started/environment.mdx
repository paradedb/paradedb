...
<Note>
  The `key_field` index option is [deprecated and
  ignored](/documentation/indexing/create-index). The Django, SQLAlchemy, and
  Rails examples create indexes with raw SQL because some client versions still
  require the argument in their index helpers.
</Note>
...
Next, let's create a ParadeDB index called `search_idx` on this table. A ParadeDB index is a covering index, which means that multiple columns can be included in the same index.
...
npm install drizzle-orm@1.0.0-rc.4 drizzle-kit@1.0.0-rc.4 postgres @paradedb/drizzle-paradedb@0.5.0 tsx
```
...
Now, in a separate terminal, push the Drizzle schema to create the ParadeDB index:
...
Generate the table migration and an empty migration for the search index:

```bash
python3 manage.py makemigrations myapp
python3 manage.py makemigrations myapp --empty --name create_search_index
```

Update the generated index migration to create the index with SQL:

```python myapp/migrations/0002_create_search_index.py
        ...
        migrations.RunSQL(
            sql="""
                ...
                USING paradedb (
                    ...
                    in_stock, metadata, created_at, last_updated_date,
                    latest_available_time, weight_range
                )
                WITH (json_fields='{"metadata":{"fast":true}}');
            """,
            reverse_sql="DROP INDEX search_idx",
        ),
    ]
```
...
def include_object(object, name, type_, reflected, compare_to):
    if type_ == "table" and reflected and name in IGNORED_TABLES:
        return False
    if type_ == "index" and name == "search_idx":
        return False
    return True
...
Next, create an empty migration for the search index.

```bash
alembic revision --rev-id 0002 -m "Create search index on mock_items"
```

Update the migration to create the search index with SQL:
...
def downgrade() -> None:
    """Downgrade schema."""
    op.execute("DROP INDEX IF EXISTS public.search_idx")
```
...
This creates `mock_items`, the ParadeDB index, and loads the mock data. Now, run the query included in `Program.cs`:
