...
<Note>
  If your primary Postgres is in a virtual private cloud (VPC), we recommend deploying ParadeDB on a compute instance
  within your VPC to avoid exposing public IP addresses and needing to provision traffic routing rules.

For the best performance, we recommend using a storage-optimized instance.
