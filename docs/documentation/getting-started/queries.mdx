...
Because `embedding`, `description`, and `rating` are part of the same ParadeDB index, the full-text match, rating filter, and nearest-neighbor ordering can all execute in a single index scan. pgvector's HNSW and IVFFlat indexes can't do this—they are standalone vector indexes that require post-filtering, which can hurt performance and recall.
