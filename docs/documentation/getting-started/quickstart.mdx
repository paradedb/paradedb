...
<Note>
  ...
  identifier within the index. Additionally, the `key_field` must be the first
  field in the list of columns. See [choosing a key
...
<Accordion title="Expected Response">
  ...
  Generic shoes | 4 | Footwear Sleek running shoes | 5 | Footwear Sturdy hiking
  boots | 4 | Footwear White jogging shoes | 3 | Footwear (5 rows) ```
</Accordion>
...
particular order. To sort by relevance, the `pdb.score` function generates BM25 scores for each row.
...
<Accordion title="Expected Response">
  ``` csv description | rating | category | score
  ----------------------+--------+----------+----------- Generic shoes | 4 |
  Footwear | 5.8135376 Sleek running shoes | 5 | Footwear | 5.4211845 White
  jogging shoes | 3 | Footwear | 5.4211845 Comfortable slippers | 3 | Footwear |
  2.9362776 Sturdy hiking boots | 4 | Footwear | 2.9362776 (5 rows) ```
</Accordion>
...
<Accordion title="Expected Response">
  ```csv order_id | product_id | order_quantity | order_total | customer_name
  ----------+------------+----------------+-------------+--------------- 1 | 1 |
  3 | 99.99 | John Doe 2 | 2 | 1 | 49.99 | Jane Smith 3 | 3 | 5 | 249.95 | Alice
  Johnson (3 rows) ```
</Accordion>
...
<Accordion title="Expected Response">
  ```csv order_id | customer_name | description
  ----------+---------------+--------------------- 3 | Alice Johnson | Sleek
  running shoes 6 | Alice Johnson | White jogging shoes 36 | Alice Johnson |
  White jogging shoes (3 rows) ```
</Accordion>
...
<Accordion title="Expected Response">
  ...
  --------------------------+------------+--------+----------- Artistic ceramic
  vase | Home Decor | 4 | [1,2,3] Designer wall paintings | Home Decor | 5 |
  [1,2,3] Handcrafted wooden frame | Home Decor | 5 | [1,2,3] (3 rows) ```
</Accordion>
