...
Faceted search enhances your search interface by providing dynamic filtering options based on document attributes. For example, an e-commerce site might show product counts by category, price range, or rating alongside search results. Let's explore how to implement this in ParadeDB.

First, let's see how many products we have in each category that match our search:
...
We can combine multiple facets to create a richer search experience. Let's get both category and rating distributions for our search results:
...
For optimal performance with large datasets, ParadeDB provides fast field optimizations. Here's the recommended way to structure a faceted search query:
...
<Note>
  For best performance, ensure that fields used in faceted aggregations are indexed as fast fields. 
  Numeric and boolean fields are automatically indexed as fast fields by default.
</Note>

You can verify which fields are indexed as fast fields using:

```sql
SELECT name, fast 
FROM paradedb.schema('search_idx') 
ORDER BY fast;
```

Faceted search can also be combined with our previous examples of full-text and similarity search to create powerful hybrid search experiences. The `@@@` operator ensures that all queries take advantage of ParadeDB's optimizations.
