...
```cs EF Core
// EF Core cannot express RANK() OVER in LINQ, so run the documented
// query directly; fusion still happens inside the database
...
var results = await dbContext.Database
    .SqlQuery<RrfResult>($"""
        ...
        vector AS (
            SELECT id, RANK() OVER (ORDER BY embedding <=> {queryEmbedding}::vector, id) AS rank
            FROM mock_items
            WHERE id @@@ pdb.all()
            ORDER BY embedding <=> {queryEmbedding}::vector, id
            LIMIT 20
        ),
        fused AS (
            SELECT id, sum(weight) AS score
...
public sealed record RrfResult(int Id, string Description, double Score);
```
