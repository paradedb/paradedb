...
This enables fast [filtering](), [Top N ordering](), and [aggregates]() over these fields.
For example, in the following index definition, `rating` and `id` are columnar indexed
...
Columnar defaults to `false` for all tokenizers besides [literal]() and [literal normalized](), which default to
`true` and do not require an explicit setting.

The reasoning is that tokenized fields can represent large documents and would be expensive to store column-wise,
whereas literal and literal normalized fields are single-value and typically much more compact.
