...
If you followed the [getting started guide](/documentation/getting-started/environment) for Django, SQLAlchemy, or Rails,
the ORM snippets below can be pasted into your Python shell or Rails console to replace the existing `search_idx` with the version shown in each example.
For a repeatable production setup, apply the same index definition through your ORM's migration system.
...
<Note>
  ...
  for more detail on how to set up your ORM's migration system to create
  indexes.
</Note>
