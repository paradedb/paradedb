...
<Note>
  The `key_field` index option is deprecated and ignored. It is still accepted
  for backward compatibility. New indexes do not require a designated key field
  or a unique first column.
</Note>
...
<Note>
  ...
  K](/documentation/sorting/topk), and
  [filtering](/documentation/filtering/overview). `USING bm25` remains supported
  as a backwards-compatible alias for `USING paradedb`.
</Note>

<Note>
  Some ORM client versions still require a `key_field` argument. The Django,
  SQLAlchemy, and Rails examples use raw SQL to create indexes without it. See
  the [getting started guide](/documentation/getting-started/environment) for
  client versions and setup instructions.
</Note>
