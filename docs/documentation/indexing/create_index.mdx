...
All code blocks in this section reference a table called `mock_items`, which was created in the [quickstart](/documentation/getting-started/quickstart). We recommend going through the quickstart
first to develop a feel for the API and create the example BM25 index.
...
<Note>
  ...
  (and not additionally indexed via a B-Tree index), note that one of the ParadeDB operators [must be used](/documentation/aggregates/limitations#filtering)
