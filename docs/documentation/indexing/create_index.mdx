...
<ParamField body="indexing_fields" required>
  A comma-separated list of columns and/or [expressions](https://www.postgresql.org/docs/current/indexes-expressional.html) to index, starting with the key field. Text, numeric, datetime, boolean, range, enum, and JSON types can be indexed.
</ParamField>
