...
## Optimizing Index Build Times

Indexes build significantly faster when ParadeDB is configured to take advantage of the underlying hardware.
These [settings](/documentation/configuration/index) are `maintenance_work_mem`, `paradedb.create_index_parallelism`, and `paradedb.create_index_memory_budget`.

```sql
-- Should not exceed the server's RAM
SET maintenance_work_mem = '16GB';
-- Should not exceed the server's CPU count
SET paradedb.create_index_parallelism = 8;
-- Should not exceed the server's RAM / paradedb.create_index_parallelism
SET paradedb.create_index_memory_budget = 4096;
```

## Choosing a Key Field

The `key_field` must be a **unique** column in the table. A non-unique key field is likely to lead to undefined behavior
