...
```sql
SELECT paradedb.is_merging('search_id');
```

An index only merges segments at the end of a INSERT/UPDATE/COPY statements, and depending on how the index is organized
at the time of the statement, segment merging _could_ take a bit of time. This function provides insight into if merging
