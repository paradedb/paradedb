...
Generally speaking, the segment count should match the server's CPU count. It can be lower if the table is very small, or higher if there are so many rows
