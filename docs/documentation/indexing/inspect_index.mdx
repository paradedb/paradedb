...
Note that Postgres index instance includes a `_bm25_index` suffix to the `index_name` passed to `paradedb.create_bm25()`.
...
```sql
SELECT size paradedb.index_size('search_idx_bm25_index');
```
