...
<Note>Stopwords Language isnt't supported with ngram tokenizer.</Note>

`stopwords_language` is a filter that removes common words from the text, which can be applied to any word tokenizer.
...
<Note>Custom Stopwords Language isnt't supported with ngram tokenizer.</Note>

`stopwords` is a filter that removes custom words from the text. The `stopwords` filter can be applied to any tokenizer.
