...
Treats the entire text as a single token, as-is. If a field is tokenized as `keyword`, using the `=` operator over that field can be pushed down to the index.
support against fields that are `text` or `varchar`.
