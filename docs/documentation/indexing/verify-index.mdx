...
The `verify_bm25_index` function performs structural integrity checks on a BM25 index:

```sql
SELECT * FROM paradedb.verify_bm25_index('search_idx');
```
...
```sql
SELECT * FROM paradedb.verify_bm25_index('search_idx', heapallindexed := true);
```
...
```sql
SELECT * FROM paradedb.verify_bm25_index('search_idx', on_error_stop := true);
```

## Parallel Verification

For very large indexes, you can distribute verification across multiple connections by specifying which segments to check.
...
```sql
SELECT * FROM paradedb.bm25_index_segments('search_idx');
```
...
FROM paradedb.bm25_index_segments('search_idx')
...
SELECT * FROM paradedb.verify_all_bm25_indexes(schema_pattern := 'public');

-- Verify indexes matching a name pattern
SELECT * FROM paradedb.verify_all_bm25_indexes(index_pattern := 'search_%');
...
| `sample_rate`     | float    | `1.0`      | Fraction of documents to check (0.0-1.0)                |
| `report_progress` | boolean  | `false`    | Emit progress messages                                  |
| `verbose`         | boolean  | `false`    | Emit detailed segment-level progress                    |
| `on_error_stop`   | boolean  | `false`    | Stop on first error found                               |
| `segment_ids`     | int[]    | `NULL`     | Specific segment indices to check (for parallelization) |

### `paradedb.verify_all_bm25_indexes`

Verifies all BM25 indexes in the database.

| Parameter         | Type    | Default | Description                                    |
| ----------------- | ------- | ------- | ---------------------------------------------- |
| `schema_pattern`  | text    | `NULL`  | Filter by schema name (SQL LIKE pattern)       |
| `index_pattern`   | text    | `NULL`  | Filter by index name (SQL LIKE pattern)        |
| `heapallindexed`  | boolean | `false` | Check that all indexed ctids exist in the heap |
| `sample_rate`     | float   | `1.0`   | Fraction of documents to check (0.0-1.0)       |
| `report_progress` | boolean | `false` | Emit progress messages                         |
| `on_error_stop`   | boolean | `false` | Stop on first error found                      |

### `paradedb.bm25_index_segments`
