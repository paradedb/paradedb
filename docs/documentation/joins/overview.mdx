...
Without range co-partitioning, parallel joins must exchange data across worker processes—either by broadcasting the build side to all workers or by shuffling both sides across workers.

When both tables participating in an equi-join have ParadeDB indexes partitioned on the join keys using [`partition_by`](/documentation/indexing/partition-by), range co-partitioning eliminates cross-worker data exchange entirely:
...
1. Worker-local joins: Rather than broadcasting or shuffling rows across workers, each parallel worker joins only its corresponding range partition of Table A with Table B locally.
2. Worker-local cardinality reduction: Downstream cardinality-reducing operations like `TopK` sorting and partial aggregations also run worker-locally before results are gathered to the leader process.
3. Physical over-partitioning: Because real-world tables almost never share identical key distributions or row counts, partition boundaries chosen during independent index builds will not naturally align. To allow the planner to align partitions effectively, we recommend [physically over-partitioning](/documentation/indexing/partition-by#physical-over-partitioning) both tables by setting `target_segment_count` to 2–8X your CPU core count (e.g. 16 to 64 segments).
