...
| Filtered vector search        |      ✅       |    ✅    | Vectors live in the same index as text and filters, so no pre- or post-filtering is required                     |
...
| `halfvec` and sparse vectors  |      ✅       |    ⚠️    | Only pgvector's `vector` type is indexable; `halfvec` is coming soon                                             |
