...
| Filtered vector search              |      ✅       |    ✅    | Vectors live in the same index as text and filters, so no separate pgvector pre- or post-filtering is required                  |
...
| Additional pgvector types           |      ✅       |    ⚠️    | Only `vector` is indexable today; `halfvec` is planned, while `sparsevec` and `bit` remain unsupported                          |
| Built-in embedding inference        |      ✅       |    ❌    | Generate embeddings in your application, or with an in-database extension, and insert them like any other column                |
