...
Spilled data is written through Postgres' own temp-file mechanism (`BufFile`), so it respects `temp_file_limit` and `temp_tablespaces`, and is cleaned up automatically at the end of the transaction - including on an early-terminated query or a crashed backend.
