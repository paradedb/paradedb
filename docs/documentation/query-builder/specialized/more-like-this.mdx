...
```cs EF Core
await dbContext
    .MockItems.Where(item => EF.Functions.MoreLikeThis(item.Id, Pdb.DocumentId(3)))
...
await dbContext
    .MockItems.Where(item =>
        EF.Functions.MoreLikeThis(item.Id, Pdb.DocumentId(3).Fields("description"))
    )
    .Select(item => new { item.Id, item.Description, item.Rating, item.Category })
    .OrderBy(item => item.Id)
    .ToListAsync();
...
await dbContext
    .MockItems.Where(item =>
        EF.Functions.MoreLikeThis(
            item.Id,
            Pdb.Document("""{"description":"Sleek running shoes","category":"footwear"}""")
        )
    )
    .Select(item => new { item.Id, item.Description, item.Rating, item.Category })
    .OrderBy(item => item.Id)
...
await dbContext
    .MockItems.Where(item =>
        EF.Functions.MoreLikeThis(item.Id, Pdb.DocumentId(3).MinTermFrequency(2))
    )
    .Select(item => new { item.Id, item.Description, item.Rating, item.Category })
    .OrderBy(item => item.Id)
    .ToListAsync();
...
await dbContext
    .MockItems.Where(item =>
        EF.Functions.MoreLikeThis(item.Id, Pdb.DocumentId(3).MinDocFrequency(3))
    )
    .Select(item => new { item.Id, item.Description, item.Rating, item.Category })
    .OrderBy(item => item.Id)
    .ToListAsync();
...
await dbContext
    .MockItems.Where(item =>
        EF.Functions.MoreLikeThis(item.Id, Pdb.DocumentId(3).MaxQueryTerms(10))
    )
    .Select(item => new { item.Id, item.Description, item.Rating, item.Category })
    .OrderBy(item => item.Id)
    .ToListAsync();
...
await dbContext
    .MockItems.Where(item =>
        EF.Functions.MoreLikeThis(item.Id, Pdb.DocumentId(3).MinWordLength(5))
    )
    .Select(item => new { item.Id, item.Description, item.Rating, item.Category })
    .OrderBy(item => item.Id)
    .ToListAsync();
...
await dbContext
    .MockItems.Where(item =>
        EF.Functions.MoreLikeThis(item.Id, Pdb.DocumentId(3).Stopwords("the", "a"))
    )
    .Select(item => new { item.Id, item.Description, item.Rating, item.Category })
    .OrderBy(item => item.Id)
    .ToListAsync();
