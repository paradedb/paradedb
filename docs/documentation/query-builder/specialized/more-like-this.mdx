...
To use this query, specify the field that identifies the reference document on the left-hand side of `@@@`
and pass its value to `pdb.more_like_this`.

<Note>
  Use a field with unique values to identify the reference document. If the
  field is not unique and multiple documents match the supplied value, the first
  matching document is used as the reference.
</Note>
...
In addition to providing a value that identifies the reference document, a custom document can also be provided as JSON.
