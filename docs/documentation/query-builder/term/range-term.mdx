...
```ruby Rails
MockItem.where("weight_range @@@ pdb.range_term(1)")
...
```ruby Rails
MockItem.where("weight_range @@@ pdb.range_term('(10, 12]'::int4range, 'Intersects')")
        .select(:id, :weight_range)
```
...
```ruby Rails
MockItem.where("weight_range @@@ pdb.range_term('(3, 9]'::int4range, 'Contains')")
        .select(:id, :weight_range)
```
...
```ruby Rails
MockItem.where("weight_range @@@ pdb.range_term('(2, 11]'::int4range, 'Within')")
        .select(:id, :weight_range)
```
