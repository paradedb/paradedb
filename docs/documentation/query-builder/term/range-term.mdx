...
<Note>
  The relation names describe the query range's perspective, which is the
  reverse of Postgres' field-centric operator naming: `Contains` matches rows
  whose range is contained by the query range (`weight_range <@ '(3, 9]'`), and
  `Within` matches rows whose range contains the query range (`weight_range @>
  '(3, 9]'`).
</Note>
