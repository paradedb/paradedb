...
MockItem.objects.filter(
    description=ParadeDB(PQ('running') | PQ('shoes'))
).values('description', 'rating', 'category').order_by('rating', 'id')[:5]
```
...
).values('description', 'rating', 'category').order_by(Lower('description'))[:5]
```
...
# Top-N optimization for JSON sort keys is not currently supported.
MockItem.objects.filter(
    description=ParadeDB(PQ('sleek') | PQ('running') | PQ('shoes'))
).values('id', 'description', 'metadata')[:5]
```
