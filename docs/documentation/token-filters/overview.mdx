...
After a [tokenizer](/documentation/tokenizers) splits up text into tokens, token filters
...
Token filters can be added to any tokenizer besides the [literal](/documentation/tokenizers/available-tokenizers/exact) tokenizer, which by definition
