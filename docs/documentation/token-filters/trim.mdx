...
This filter is useful for tokenizers that don't already split on whitespace, like the [normalized](/documentation/tokenizers/available-tokenizers/normalized)
tokenizer or certain language-specific tokenizers.

```sql
CREATE INDEX search_idx ON mock_items
USING bm25 (id, (description::pdb.normalized('trim=true')))
WITH (key_field='id');
```
...
```sql
SELECT
  '    token with whitespace   '::pdb.normalized::text[],
  '    token with whitespace   '::pdb.normalized('trim=true')::text[];
```
