...
<Note>
  ...
  recommend passing a `LIMIT` to any query where `paradedb.snippet` is called to
...
<Accordion title="Expected Response">
  ```csv id | snippet | snippet_positions
  ----+----------------------------+------------------- 3 | Sleek running{" "}
  <b>shoes</b> | {"{14,19}"}4 | White jogging <b>shoes</b> | {"{14,19}"}5 |
  Generic <b>shoes</b> | {"{8,13}"}
  (3 rows) ```
</Accordion>
