...
To understand exactly how this works, let's consider the following two term queries:
...
| SLeeK RUNNING ShOeS | `sleek` `running` `shoes` | ✅    | Contains the token `running`.         | [Lowercasing](/documentation/v2/indexing)  |
| Sleek run shoe      | `sleek` `run` `shoe`      | ❌    | Does not contain the token `running`. | [Stemming](<(/documentation/v2/indexing)>) |
