...
When one of ParadeDB's [search operators](/documentation/filtering/indexed) is present at the same level as the `ORDER BY ... LIMIT`, ParadeDB can accelerate the vector search query.
...
To search within a subset of rows, replace `pdb.all()` with a real predicate using any of the [search operators](/documentation/filtering/indexed#non-text-columns).
