...
- **Clusters that cannot possibly help are skipped.** From per-cluster radii
  stored at index time, the index can prove that every vector a cluster owns
  is farther from the query than the neighbors already found, and skip it
  without reading it. A skipped cluster costs only its opening share, so the
  same budget reaches more of the clusters that can actually contribute. The
  proof is exact, so this costs no recall.
...
A query stops for exactly one of two reasons: it spent its budget, or it ran
out of clusters. Skipping changes how far the budget goes, never when the
query stops.
