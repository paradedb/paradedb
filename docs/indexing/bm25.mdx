...
WITH (
  text_fields='{
    "description": {"tokenizer": "en_stem"}, "category": {"fast": true}
  }'
);
```
...
<Note>`ngrams` and `chinese_compatible` tokenizers are coming soon.</Note>
...
<ParamField body="lowercase">
  Applies a lowercase transformation on the text without tokenization.
</ParamField>
