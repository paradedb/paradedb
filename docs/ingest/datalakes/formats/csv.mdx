...
For instance, `s3://bucket/folder/file.csv` if the file is in Amazon S3, `https://domain.tld/file.csv`
if the file is on a HTTP server, or `/path/to/file.csv` if the file is on the local file system.
</ParamField>
