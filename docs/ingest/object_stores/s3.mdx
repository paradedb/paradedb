...
Note: Providing credentials via `key_id` and `secret` requires permanent AWS IAM/Identity Center keys. To connect via ephemeral
key and secret credentials, please refer to (/ingest/object_stores/s3#credential-chain-provider).
