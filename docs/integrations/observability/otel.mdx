...
<ParamField body="traces_table_name" required>
  Name of the Postgres table within the provided database where the
  OpenTelemetry traces will be stored. It must be a valid table name which is
  then created by the exporter.
</ParamField>
