...
ParadeDB provides <a href="https://github.com/destrex271/postgresexporter">an unofficial OpenTelemetry (OTEL) exporter</a> that enables ingesting telemetry data into ParadeDB.
...
<ParamField body="logs_table_name" required>
  Name of the table where OpenTelemetry (OTel) logs will be stored. Can be any valid identifier for table name.
</ParamField>

<ParamField body="traces_table_name" required>
  Name of the table where OpenTelemetry (OTel) traces will be stored. Can be any valid identifier for table name.
</ParamField>

<ParamField body="host" default="localhost" required>
  TCP/IP address on which the server listens for connections from client applications
</ParamField>
