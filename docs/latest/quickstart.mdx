...
The `rank_hybrid` function accepts a BM25 query and a similarity query. It normalizes the BM25 and similarity scores
...
Let's join this table with our `mock_items` table to see the results of our hybrid search:

```sql
SELECT mock_items.description, mock_items.category, mock_items.embedding, search.rank_hybrid
FROM mock_items
LEFT JOIN (
    SELECT * FROM search_idx.rank_hybrid(
        bm25_query => 'description:keyboard OR category:electronics',
...
) search ON mock_items.id = search.id
LIMIT 5;
```
