...
Let's execute a hybrid search that calculates the similarity and BM25 scores for each row, normalizes
...
WITH similarity AS (
  SELECT
    id,
    description,
    1 - ((embedding <-> '[1,2,3]') - MIN('[1,2,3]' <-> embedding) OVER ()) /
      (MAX('[1,2,3]' <-> embedding) OVER () - MIN('[1,2,3]' <-> embedding) OVER ()) AS score
  FROM mock_items
  ORDER BY embedding <-> '[1,2,3]'
  LIMIT 20
),
bm25 AS (
  SELECT s.id, s.description, r.minmax_bm25 as score
  FROM ngrams_idx.search('description:keyboard') as s
  LEFT JOIN ngrams_idx.rank_minmax('description:keyboard') as r ON s.id = r.id
  LIMIT 20
)
SELECT
    COALESCE(similarity.description, bm25.description) as description,
    COALESCE(similarity.score, 0.0) * 0.5 + COALESCE(bm25.score, 0.0) * 0.5 AS score_hybrid
FROM similarity
FULL OUTER JOIN bm25 ON similarity.id = bm25.id
