...
The term query should not be confused with the [match](/legacy/advanced/full-text/match) query, which auto-tokenizes
the query string.
...
`term` can be used to filter over custom Postgres [enums](/legacy/indexing/field-options#enumerated-types)
