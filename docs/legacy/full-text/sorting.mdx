...
The `score` column returned by [`pdb.score`](/legacy/full-text/scoring) can be used to sort results by
BM25 relevance.
...
An `ORDER BY...LIMIT` over a single [text](/legacy/indexing/field-options#text-fields), [numeric](/legacy/indexing/field-options#numeric-fields),
[datetime](/legacy/indexing/field-options#datetime-fields), or [boolean](/legacy/indexing/field-options#boolean-fields) field is automatically "pushed down"
to the BM25 index if the `ORDER BY` field is indexed as [fast](/legacy/indexing/fast-fields). This makes these queries significantly faster.
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=10.00..34.03 rows=5 width=36)
   ->  Custom Scan (ParadeDB Base Scan) on mock_items  (cost=10.00..34.03 rows=5 width=36)
         Table: mock_items
         Index: search_idx
         Scores: false
            Sort Field: rating
...
If a fast text field is indexed with the `raw` [normalizer](/legacy/indexing/fast-fields#normalizers), `ORDER BY <text_field> LIMIT` can be pushed down.

If the `lowercase` [normalizer](/legacy/indexing/fast-fields#normalizers) is used, then `ORDER BY lower(<text_field>) LIMIT` (but not `ORDER BY <text_field> LIMIT`)
can be pushed down.
...
---------------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=10.00..34.05 rows=5 width=584)
   ->  Custom Scan (ParadeDB Base Scan) on mock_items  (cost=10.00..34.05 rows=5 width=584)
         Table: mock_items
         Index: search_idx
         Scores: false
            Sort Field: category
...
You can verify if partial ORDER BY pushdown occurred by running `EXPLAIN` on the query. The query plan will show a `Custom Scan` with our ParadeDB Base Scan provider, followed by an appropriate sort operation based on your PostgreSQL version:
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
  Limit  (cost=0.19..0.34 rows=10 width=44) (actual time=1.113..1.114 rows=0 loops=1)
   ->  Incremental Sort  (cost=0.19..0.34 rows=10 width=44) (actual time=1.113..1.114 rows=0 loops=1)
         Sort Key: (pdb.score(id)), sale_date, amount
         Presorted Key: (pdb.score(id))
         Full-sort Groups: 1  Sort Method: quicksort  Average Memory: 25kB  Peak Memory: 25kB
         ->  Custom Scan (ParadeDB Base Scan) on sales  (cost=0.00..0.03 rows=10 width=44) (actual time=1.055..1.055 rows=0 loops=1)
               Table: sales
               Index: sales_index
               Segment Count: 1
               Heap Fetches: 0
