...
<Note>
  ...
  [index tuning](/documentation/performance-tuning/overview) for details.
</Note>
...
USING paradedb (<columns_or_expressions>)
...
<Note>
  ...
  used](/documentation/aggregates/limitations#paradedb-operator) in order for filtering
...
USING paradedb (id, description, lower(description))
WITH (key_field='id');
```
