...
In addition to text and JSON, ParadeDB exposes options for numeric, boolean, range, and enum fields.
...
<Note>
  As of `0.24.1`, `datetime_fields` is deprecated. The performance improvements
  it used to enable are now on by default, so the option should be removed from
  your `CREATE INDEX` statement.
</Note>

Columns of type `DATE`, `TIMESTAMP`, `TIMESTAMP WITH TIME ZONE`, `TIME`, `TIME WITH TIME ZONE`, and their
corresponding array types are indexed and made fast automatically — no configuration is required.
...
Options for columns of type `int4range`, `int8range`, `numrange`, `daterange`, `tsrange`, and `tstzrange` should be
passed to `range_fields`. Range fields should be queried with [range term queries](/legacy/advanced/term/range-term).
