...
For documents containing multiple languages, you can specify multiple stemmers as an array.
...
WITH (
    key_field='id',
    text_fields='{
        "description": {"tokenizer": {"type": "default", "stemmer": ["English", "French"]}}
    }'
);
```

With multiple stemmers, searching for "studies" will match "studying", and searching for "courir" (French: to run) will match "courant" (French: running).
