...
canonical: https://www.paradedb.com/docs/operate/deploy/cloud-platforms/fly
---

<Note>
  This guide deploys ParadeDB Community as a single-node container. Use
  [ParadeDB Enterprise](/operate/deploy/enterprise) if ParadeDB indexes need
  physical replication for high availability, failover, or read replicas that
  can serve ParadeDB queries. Enterprise can be deployed via
  [Kubernetes](/operate/deploy/self-hosted/kubernetes) or
  [BYOC](/operate/deploy/byoc).
</Note>
...
`fly launch` prompts for an app name and a primary region, then writes a `fly.toml` in the current directory. See the [Docker Hub page](https://hub.docker.com/r/paradedb/paradedb/tags) for available `paradedb/paradedb` tags — pin one (for example `paradedb/paradedb:{version}-pg18`) instead of `latest` for reproducible deploys.
