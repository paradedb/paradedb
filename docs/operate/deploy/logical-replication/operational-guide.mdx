...
canonical: https://www.paradedb.com/docs/operate/deploy/logical-replication/operational-guide
---
...
Replication](/operate/deploy/logical-replication/getting-started) to create
...
<Note>
  ...
  guide, build the ParadeDB indexes you plan to query on the ParadeDB
  subscriber.
</Note>
...
### 2. Build ParadeDB Indexes on ParadeDB

Once the replicated tables are caught up, create ParadeDB indexes locally on
ParadeDB:
...
See [Reindexing](/operate/index-maintenance/reindexing) for the ParadeDB index rebuild
workflow.
...
Changes](/operate/deploy/logical-replication/configuration). For multiple source
databases, see [Multi-Database Replication for
Microservices](/operate/deploy/logical-replication/multi-database).
