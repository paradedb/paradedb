...
canonical: https://www.paradedb.com/docs/operate/deploy/self-hosted/extension
---

<Note>
  ParadeDB Community can run as a single-node extension install. Use [ParadeDB
  Enterprise](/operate/deploy/enterprise) when ParadeDB indexes need physical
  replication for high availability, failover, or read replicas that can serve
  ParadeDB queries.
</Note>
...
Install the `pg_search` extension, which powers ParadeDB's search features.
...
If you are a [ParadeDB Enterprise](/operate/deploy/enterprise) user, you should have received a copy of the enterprise binaries. Please [contact sales](mailto:sales@paradedb.com) for access.
