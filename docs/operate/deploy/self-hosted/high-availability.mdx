...
canonical: https://www.paradedb.com/docs/operate/deploy/self-hosted/high-availability
---

High availability (HA) minimizes downtime when a Postgres instance fails. For
ParadeDB queries, HA requires [ParadeDB Enterprise](/operate/deploy/enterprise)
because the ParadeDB index must be physically replicated to standby instances.
Deploy Enterprise inside a [CNPG Kubernetes
cluster](/operate/deploy/self-hosted/kubernetes) to run this topology.
...
Between physical replicas, ParadeDB requires the use of a few settings (which are automatically set by [CNPG](/operate/deploy/self-hosted/kubernetes)) in order to avoid query cancellation due to ongoing reorganization of the data on the primary replica.
