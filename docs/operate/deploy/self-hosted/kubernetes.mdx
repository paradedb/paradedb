...
canonical: https://www.paradedb.com/docs/operate/deploy/self-hosted/kubernetes
---
...
<Note>
  ...
  [ParadeDB Enterprise](/operate/deploy/enterprise) when ParadeDB indexes need
  physical replication for [high
  availability](/operate/deploy/self-hosted/high-availability), failover, or
...
You need permission to install the CloudNativePG operator and ParadeDB Helm chart, plus a StorageClass that provisions persistent volumes suitable for PostgreSQL. For production, use SSD-backed block storage sized for your data and indexes.

This guide also applies to clusters managed by platforms that support custom Helm charts, such as [Porter](https://docs.porter.run/addons/custom-helm-charts), [Qovery](https://www.qovery.com/docs/configuration/helm), and [Rancher](https://ranchermanager.docs.rancher.com/how-to-guides/new-user-guides/helm-charts-in-rancher). Configure access to your cluster before running the commands below. You remain responsible for database backups, upgrades, and monitoring.
