...
canonical: https://www.paradedb.com/docs/operate/deploy/third-party-extensions
---
...
The ParadeDB Docker image ships with the following extensions pre-installed. The set is kept small
to keep the image size manageable; other extensions can be installed separately.

- **`pg_search`** — BM25 search, vector search, hybrid search, facets, and aggregates
- **`pgvector`** — The `vector` type used by ParadeDB vector indexes and standalone pgvector indexes
...
- **[Citus](/operate/deploy/citus)** — Distributed Postgres for horizontal scaling
