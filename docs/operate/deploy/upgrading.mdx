...
canonical: https://www.paradedb.com/docs/operate/deploy/upgrading
---
...
<Note>
  ...
  vector search](/reference/vector/overview). This extension is not managed by
...
The latest version of `pg_search` is `{version}`. Please refer to the [releases](https://github.com/paradedb/paradedb/releases) page for all available versions of `pg_search`.
...
5. If you are using [ParadeDB BYOC](/operate/deploy/byoc), an automatic rollout will begin. One by one, the pods will be restarted to apply the new version of the extension.
...
```bash
docker pull paradedb/paradedb:{version}
```

The latest version of the Docker image should be `{version}`.
...
```sql
ALTER EXTENSION pg_search UPDATE TO '{version}';
```
