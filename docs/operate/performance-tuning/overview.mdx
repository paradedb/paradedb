...
canonical: https://www.paradedb.com/docs/operate/performance-tuning/overview
---
...
If ParadeDB is deployed with [CloudNativePG](/operate/deploy/self-hosted/kubernetes) using the [ParadeDB Helm chart](https://github.com/paradedb/charts), configure these settings under `cluster.postgresql.parameters` in your `values.yaml` file. Parameter values must be strings. For example:
