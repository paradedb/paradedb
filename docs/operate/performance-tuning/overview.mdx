...
If ParadeDB is deployed with [CloudNativePG](/operate/deploy/self-hosted/kubernetes), configure these settings under `cluster.postgresql.parameters` in your `values.yaml` file. Parameter values must be strings. For example:
