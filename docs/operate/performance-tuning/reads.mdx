...
canonical: https://www.paradedb.com/docs/operate/performance-tuning/reads
---
...
If an index experiences frequent writes, the search performance of some queries like [sorting](/reference/full-text/score) or
[aggregates](/reference/aggregates/overview) can degrade if `VACUUM` has not been recently run. This is because writes can cause parts of Postgres' visibility map
...
[`max_parallel_workers_per_gather`](/operate/performance-tuning/reads#raise-parallel-workers). If `max_parallel_workers_per_gather` is greater than the number of CPUs on the host machine, then increasing the target segment count to match `max_parallel_workers_per_gather` can improve query
performance.

<Note>
  Actual segment counts may vary depending on parallel builder distribution, memory limits, and table size.

When partitioning an index with [`partition_by`](/reference/indexing/partition-by), we recommend physically over-partitioning by setting `target_segment_count` to 2–8X `max_parallel_workers_per_gather` (or CPU count). This allows filtered queries to eliminate a larger fraction of non-matching data and enables the planner to align logical partition boundaries across tables during joins, balanced against segment scan and metadata overhead.
