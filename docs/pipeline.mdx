...
In this Quickstart we will build a pipeline to sync documents stored in AWS Aurora Postgres with their embeddings,
stored in ElasticSearch. As you will see, Retake supports a variety of sources and sinks and
...
`faqs`, which contains two columns: `questions` and `answers`. We wish to combine each question and answer, embed it, and store it in ElasticSearch.
...
In this example, `Source` is our AWS Aurora Postgres database.
You can find a list of all available sources [here]().
...
In this example, we wish to concatenate `question` and `answer`. Other `Source` classes may have different function signatures, which
are documented [here]().
...
`Embedding` specifies the embedding model we wish to use. Retake provides wrappers around most popular embedding models
and you can also bring your own — documentation [here]().
...
`Sink` specifies where we want to store our vectors. In this example, `Sink` is ElasticSearch. You can find
a list of all available sinks [here]().
...
a list of all available targets [here]().
...
`Pipeline` defines your entire vector data stream, from source to sink. You can read more [here]().
