---
title: 0.17.0
description: "ParadeDB release notes for 0.17.0"
noindex: true
---

## Pushdown for `COUNT`

We're making rapid progress on our [roadmap](/project/roadmap#deeper-analytics-improvements) to fully push down aggregates to the BM25 index. The first aggregate we've pushed down
is `COUNT`.
...
Prior to this release, the compaction, or merging, of the index [LSM tree](/concepts/architecture#lsm-tree) happened in the foreground, blocking
...
With this release, merging large layers happens in the background. This is configurable with the `background_layer_sizes` [index option](/operate/performance-tuning/writes)
...
The full changelog is available [on the GitHub Release](https://github.com/paradedb/paradedb/releases/tag/v0.17.0).
