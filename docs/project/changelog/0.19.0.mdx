---
title: 0.19.0
description: "ParadeDB release notes for 0.19.0"
noindex: true
---
...
The [custom aggregate scan](/reference/aggregates/overview) has pushdown support for several new query types. The first is `FILTER`,
...
We have made significant progress on the new v2 API. A non-exhaustive list of additions includes:

- A new way to specify index configuration options like [tokenizers](/reference/tokenizers/overview) and [token filters](/reference/token-filters/overview) without the
  old JSON string syntax
- First class support for [multiple tokenizers per field](/reference/tokenizers/multiple-per-field)
- New token filters like [alpha numeric only](/reference/token-filters/alphanumeric) and [token length](/reference/token-filters/token-length)
- A new way to cast queries to [fuzzy](/reference/full-text/fuzzy), [slop](/reference/full-text/phrase#adding-slop), and [boost](/reference/full-text/relevance-tuning)
- An improved [more like this](/reference/full-text/more-like-this) function
