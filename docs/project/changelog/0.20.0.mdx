---
title: 0.20.0
description: "ParadeDB release notes for 0.20.0"
noindex: true
---
...
ParadeDB 0.20 introduces powerful [search aggregation capabilities](/reference/aggregates/overview) through the new `pdb.agg()` function. This function can be used in two ways to push analytics down into the Tantivy index for optimal performance.

The first is as a **window function** for fast faceting alongside Top K queries:
...
Standard Postgres aggregations like [`COUNT(*)`](/reference/aggregates/metrics/count) are automatically routed to use this optimized path when possible.

### v2 API

The v2 API has reached feature parity (except for custom stopwords and dismax) with the legacy API.

It has now been promoted to the default API and can be found in the current [indexing reference](/reference/indexing/create-index).
...
- Columnar storage by default for all non-text and literal tokenized types, removing the need to configure manually.
...
**Improved Default Tokenizer**: The default [tokenizer](/reference/tokenizers/overview) has been changed to `pdb.unicode_words`, which splits text based on the [Unicode Standard Annex #29](http://www.unicode.org/reports/tr29/) rules for better international text support.
...
**New Token Filters**: Added a `trim` [token filter](/reference/token-filters/overview) that removes leading and trailing whitespace from tokens, improving search precision.

## Performance Improvements 🚀

### Write Throughput

We have made significant improvements to ParadeDB's [write throughput](/operate/performance-tuning/writes) through two major architectural changes. The first is enabling [mutable segments](https://github.com/paradedb/paradedb/pull/3516) by default, which are designed to incur minimal overhead during single-row writes. The overhead of tokenizing, serializing, and flushing an immutable segment is now completely eliminated for these operations.
