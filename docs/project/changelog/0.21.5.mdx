---
title: 0.21.5
description: "ParadeDB release notes for 0.21.5"
noindex: true
---
...
The [literal normalized](/reference/tokenizers/available-tokenizers/literal-normalized) tokenizer now gets indexed
as columnar by default (in addition to the inverted index). This means that literal normalized fields can take part in [aggregate](/reference/aggregates/overview) and [Top K](/reference/full-text/top-k)
queries without the need for a separate [literal](/reference/tokenizers/available-tokenizers/literal) tokenizer.
