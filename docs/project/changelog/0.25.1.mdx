---
title: 0.25.1
description: "ParadeDB release notes for 0.25.1"
noindex: true
---
...
the work-budget ceiling, is now the only [recall/latency setting](/reference/vector/tuning).

### Deterministic Vector Search Results

Tiebreaker columns can now follow the distance in `ORDER BY`, making [vector search results deterministic](/reference/vector/querying#deterministic-results)
...
The vector arm of [reciprocal rank fusion](/reference/hybrid/rrf) queries is now pushed down into the ParadeDB
