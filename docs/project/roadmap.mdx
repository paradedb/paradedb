...
canonical: https://www.paradedb.com/docs/project/roadmap
---

We're a lean team that likes to ship at [incredibly high velocity](https://github.com/paradedb/paradedb/releases). For features and improvements that have already shipped, see the [changelog](https://github.com/paradedb/paradedb/releases).

## In Progress

### Vector Search

- **[Native vector search](/reference/vector/overview) (beta)**. The ParadeDB index natively indexes pgvector's `vector` type, addressing pgvector's limitations around filtered queries — specifically, queries that combine vector similarity with metadata filters or full-text search predicates.
...
- **Join pushdown (beta)**. [Join pushdown](/reference/joins/overview) pushes search predicates directly into the index for significantly better performance.
...
- **PaaS Providers**. Official tutorials for hosting ParadeDB on more platform-as-a-service providers like Heroku and others are coming. [Railway](/operate/deploy/cloud-platforms/railway), [Render](/operate/deploy/cloud-platforms/render), [Fly.io](/operate/deploy/cloud-platforms/fly), [DigitalOcean](/operate/deploy/cloud-platforms/digitalocean), and [Dokku](/operate/deploy/cloud-platforms/dokku) are already available.

### Managed Cloud

- Today, you can [deploy ParadeDB](/operate/deploy/overview) self-hosted, on cloud platforms, or with ParadeDB BYOC. We're also building ParadeDB Cloud, a fully managed ParadeDB: first-class search from Postgres, with a developer experience to match. [Join the waitlist](https://www.paradedb.com/cloud) to get notified when it launches.
