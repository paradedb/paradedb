...
In this query, we first use `paradedb.score_bm25` to calculate each row's BM25 score with respect to 
the query "keyboard." Next, we use the `<->` operator to compute a similarity score. Finally, we combine 
the two scores as a weighted average, assigning a weight of `0.8` to the first BM25 score and a weight of 
`0.2` to the latter score.
