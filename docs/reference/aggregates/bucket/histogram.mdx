...
canonical: https://www.paradedb.com/docs/reference/aggregates/bucket/histogram
---
...
await db
  .select({
    agg: search.agg({ histogram: { field: "rating", interval: "1" } }),
  })
  .from(mockItems)
...
await dbContext
    ...
    .Select(item => EF.Functions.Agg(new { histogram = new { field = "rating", interval = "1" } }))
    .ToListAsync();
```
