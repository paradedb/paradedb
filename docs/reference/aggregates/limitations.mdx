...
`GROUP BY DATE(created_at)` and `GROUP BY created_at::date` support aggregate pushdown when `created_at` is a bare `timestamp without time zone` column that is [columnar indexed](/reference/indexing/columnar). The usual aggregate pushdown requirements still apply, including a ParadeDB operator in the query.
...
A few `pdb.agg()` shapes run on a single table but not over a join: metric aggregations over JSON sub-fields, such as `{"sum": {"field": "metadata.qty"}}`, `terms` on a JSON sub-field that holds numbers, `min_doc_count: 0`, which lists every term of the column, and a field indexed from a computed expression, such as `(upper(name)::pdb.literal('alias=name_upper'))`. A field indexed through a plain tokenizer cast, such as `(name::pdb.literal)`, works over a join like the column itself.
