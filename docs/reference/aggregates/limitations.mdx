...
- A `STRING_AGG` separator cannot be reduced to a constant during planning, such as when it is a column, a non-foldable function expression like `current_setting(...)`, or a parameter in a generic plan
