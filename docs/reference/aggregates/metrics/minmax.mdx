...
canonical: https://www.paradedb.com/docs/reference/aggregates/metrics/minmax
---

`min` and `max` return the smallest and largest values of a column, respectively.

SQL's `MIN`/`MAX` syntax is supported in beta and is enabled by default. To disable it, run:
