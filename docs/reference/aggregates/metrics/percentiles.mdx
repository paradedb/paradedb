...
canonical: https://www.paradedb.com/docs/reference/aggregates/metrics/percentiles
---
...
await db
  .select({
    agg: search.agg({ percentiles: { field: "rating", percents: [50, 95] } }),
  })
  .from(mockItems)
...
await dbContext
    ...
    .Select(item =>
        EF.Functions.Agg(new { percentiles = new { field = "rating", percents = new[] { 50, 95 } } })
    )
    .ToListAsync();
```
