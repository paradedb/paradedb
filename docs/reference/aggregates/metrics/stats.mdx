...
canonical: https://www.paradedb.com/docs/reference/aggregates/metrics/stats
---
...
await db
  .select({
    agg: search.agg({ stats: { field: "rating" } }),
  })
  .from(mockItems)
...
await dbContext
    ...
    .Select(item => EF.Functions.Agg(new { stats = new { field = "rating" } }))
    .ToListAsync();
```
