...
canonical: https://www.paradedb.com/docs/reference/aggregates/metrics/tophits
---

The top hits aggregation is meant to be used in conjunction with the [terms](/reference/aggregates/bucket/terms)
        ...
        EF.Functions.Agg(
            new
            {
                top_hits = new
                {
                    ...
                    docvalue_fields = new[] { "id", "created_at" }
                }
            }
        )
    )
...
If a text or JSON field is passed to `docvalue_fields`, it must be indexed with the [literal](/reference/tokenizers/available-tokenizers/literal)
or [literal normalized](/reference/tokenizers/available-tokenizers/literal-normalized) tokenizer.
    ...
    .Select(group =>
        EF.Functions.Agg(
            new
            {
                ...
                {
                    size = 3,
                    sort = new object[] { new { created_at = "desc" }, new { id = "asc" } },
                    docvalue_fields = new[] { "id", "created_at" }
                }
            }
        )
