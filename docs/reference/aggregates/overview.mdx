---
title: Aggregates
description: Accelerate SQL and JSON aggregates with the ParadeDB index
canonical: https://www.paradedb.com/docs/reference/aggregates/overview
---

ParadeDB can execute aggregate work using the [columnar](/concepts/architecture#columnar-index) portion of the ParadeDB index, which can significantly accelerate performance compared to vanilla Postgres.
...
If a query does not otherwise need a full-text predicate, add [`pdb.all()`](/reference/full-text/all) to make the query eligible for aggregate pushdown:
...
See [Limitations](/reference/aggregates/limitations) for the full list of supported aggregate shapes and fallback cases.
