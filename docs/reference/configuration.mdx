...
| `paradedb.enable_range_partitioned_join`       | `on`         | user    | Enables range co-partitioned joins and asymmetric range alignment for MPP joins on indexes configured with `partition_by`. |
