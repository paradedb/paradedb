...
description: Push filters into the ParadeDB index scan
canonical: https://www.paradedb.com/docs/reference/filtering/indexed
---

Indexed columns are columns or expressions included in the ParadeDB index.
For filters over built-in scalar types and literal text columns, this is the most efficient path because the filter can be pushed directly into the ParadeDB scan.
For filters backed by another database index, see [External Indexes](/reference/filtering/external-indexes).

Adding filters to a search query is as simple as using regular SQL `WHERE` clauses.
...
| Operator | Meaning                                                                                       |
| -------- | --------------------------------------------------------------------------------------------- |
| `@@@`    | [General search condition](/concepts/full-text/overview)                                      |
| `\|\|\|` | [Match disjunction](/reference/full-text/match#match-disjunction) — contains any of the terms |
| `&&&`    | [Match conjunction](/reference/full-text/match#match-conjunction) — contains all of the terms |
| `===`    | [Term](/reference/full-text/term) — exact token match                                         |
| `###`    | [Phrase](/reference/full-text/phrase) — tokens in order and position                          |

For queries that do not require text search, add [`pdb.all`](/reference/full-text/all) to force the ParadeDB
...
While not required, filtering performance over non-text columns can be improved by including them in the ParadeDB index.
...
For example, if `rating` and `created_at` are frequently used in filters, they can be added to the ParadeDB index during index creation:
...
To push down the `category = 'Footwear'` filter, `category` must be indexed using the [literal](/reference/tokenizers/available-tokenizers/literal) tokenizer:
