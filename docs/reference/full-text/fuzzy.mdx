...
canonical: https://www.paradedb.com/docs/reference/full-text/fuzzy
---
...
Fuzziness is supported for [match](/reference/full-text/match) and [term](/reference/full-text/term) queries.
...
fuzzy_or_stmt = (
    ...
    .where(search.match_any(MockItem.description, search.fuzzy("runing shose", 2)))
    .limit(5)
)
...
fuzzy_and_stmt = (
    select(MockItem.id, MockItem.description)
    .where(search.match_all(MockItem.description, search.fuzzy("runing shose", 2)))
    .limit(5)
)
...
fuzzy_term_stmt = (
    select(MockItem.id, MockItem.description)
    .where(search.term(MockItem.description, search.fuzzy("shose", 2)))
    .limit(5)
)
...
MockItem.search(:description)
        .match_any(ParadeDB.fuzzy('runing shose', 2))
        .select(:id, :description)
        .limit(5)

# Fuzzy match conjunction
MockItem.search(:description)
        .match_all(ParadeDB.fuzzy('runing shose', 2))
        .select(:id, :description)
        .limit(5)
...
By default, the [match](/reference/full-text/match) and [term](/reference/full-text/term) queries require exact token matches between the query and indexed text. When a query is cast to `fuzzy(n)`, this requirement is relaxed -- tokens are matched if their Levenshtein distance, or edit distance, is less than or equal to `n`.
...
- **Substitution** replaces one character with another e.g., "shoos" → "shoes" (substitute "o" → "e") has an edit distance of `1`

Swapping two adjacent characters — a **transposition** — is not a distinct operation by default. "shose" → "shoes" costs two
substitutions, so it has an edit distance of `2`. This can be lowered to `1`, see [transposition cost](#transposition-cost).
...
stmt = (
    select(MockItem.id, MockItem.description)
    .where(search.term(MockItem.description, search.fuzzy("rann", 1, prefix=True)))
    .limit(5)
)
...
```ruby Rails
MockItem.search(:description)
        .term(ParadeDB.fuzzy("rann", 1, prefix: true))
        .select(:id, :description)
        .limit(5)
```
...
When used with [match](/reference/full-text/match) queries, fuzzy prefix treats all tokens in the query string as prefixes.
...
await db
  ...
  .from(mockItems)
  .where(
    search.matchAll(mockItems.description, search.fuzzy("slee rann", 1, true)),
  )
  .limit(5);
```
...
stmt = (
    select(MockItem.id, MockItem.description)
    .where(search.match_all(MockItem.description, search.fuzzy("slee rann", 1, prefix=True)))
    .limit(5)
)
...
```ruby Rails
MockItem.search(:description)
        .match_all(ParadeDB.fuzzy("slee rann", 1, prefix: true))
        .select(:id, :description)
        .limit(5)
```
...
await db
  ...
  .from(mockItems)
  .where(
    search.term(mockItems.description, search.fuzzy("shose", 1, false, true)),
  )
  .limit(5);
```
...
stmt = (
    select(MockItem.id, MockItem.description)
    .where(search.term(MockItem.description, search.fuzzy("shose", 1, transpose_cost_one=True)))
    .limit(5)
)
...
```ruby Rails
MockItem.search(:description)
        .term(ParadeDB.fuzzy("shose", 1, transposition_cost_one: true))
        .select(:id, :description)
        .limit(5)
```
