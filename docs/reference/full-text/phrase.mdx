...
canonical: https://www.paradedb.com/docs/reference/full-text/phrase
---

Phrase queries work exactly like [match conjunction](/reference/full-text/match#match-conjunction), but are more strict in that they require the
...
| Sleek shoes running | `sleek` `shoes` `running` | ❌    | `running` and `shoes` not in the right order.  | [Match conjunction](/reference/full-text/match#match-conjunction) |
| SLeeK RUNNING ShOeS | `sleek` `running` `shoes` | ✅    | Contains `running` and `shoes`, in that order. | [Lowercasing](/reference/indexing/create-index)                   |
...
stmt = (
    select(MockItem.description, MockItem.rating, MockItem.category)
    .where(search.phrase(MockItem.description, search.slop("shoes running", 2)))
)
...
```ruby Rails
MockItem.search(:description)
        .phrase(ParadeDB.slop("shoes running", 2))
        .select(:description, :rating, :category)
```
...
stmt = (
    select(MockItem.description, MockItem.rating, MockItem.category)
    .where(search.phrase(MockItem.description, search.tokenize("running shoes", tokenizer.whitespace())))
)
...
The following query matches documents containing the token `running` immediately followed by `shoes`:
