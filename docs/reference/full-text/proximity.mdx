...
canonical: https://www.paradedb.com/docs/reference/full-text/proximity
---
...
await dbContext
    .MockItems.Where(item =>
        EF.Functions.Proximity(item.Description, Pdb.Proximity("sleek").Within(1, "shoes"))
    )
...
<Note>
  Like the [term](/reference/full-text/term) query, the query string in a
...
await dbContext
    .MockItems.Where(item =>
        EF.Functions.Proximity(item.Description, Pdb.ProximityRegex("sl.*").Within(1, "shoes"))
    )
    .Select(item => new { item.Description, item.Rating, item.Category })
    .ToListAsync();
```
...
await dbContext
    .MockItems.Where(item =>
        EF.Functions.Proximity(item.Description, Pdb.ProximityRegex("sl.*", 100).Within(1, "shoes"))
    )
    .Select(item => new { item.Description, item.Rating, item.Category })
    .ToListAsync();
```
