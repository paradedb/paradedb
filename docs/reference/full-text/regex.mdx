...
canonical: https://www.paradedb.com/docs/reference/full-text/regex
---
...
await db
  ...
  .from(mockItems)
  .where(search.regex(mockItems.description, "key.*"));
```
...
```cs EF Core
await dbContext
    .MockItems.Where(item => EF.Functions.Regex(item.Description, "key.*"))
...
<Note>
  ...
  text, use the [`literal`](/reference/tokenizers/available-tokenizers/literal)
  tokenizer.
</Note>
