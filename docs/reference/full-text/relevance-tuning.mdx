...
You can configure BM25 parameters per field when creating an index.
...
Pass `k1` and `b` as options to a field's [tokenizer](/reference/tokenizers/overview):
...
Omitted parameters keep their defaults. Queries automatically use the indexed values.
