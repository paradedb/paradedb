...
canonical: https://www.paradedb.com/docs/reference/full-text/term
---

Term queries look for exact token matches. A term query is like an exact string match, but at the
token level.

Unlike [match](/reference/full-text/match) or [phrase](/reference/full-text/phrase) queries, term queries treat the query
...
<Note>
  ...
  [lowercase](/reference/token-filters/lowercase) tokens by default. Make sure
  to account for this when searching for a term.
</Note>

<Note>
  If you are using `===` to do an exact string match on the original text, make
  sure that the text uses the
  [literal](/reference/tokenizers/available-tokenizers/literal) tokenizer.
</Note>
...
| SLeeK RUNNING ShOeS | `sleek` `running` `shoes` | ✅    | Contains the token `running`.         | [Lowercasing](/reference/indexing/create-index) |
| Sleek run shoe      | `sleek` `run` `shoe`      | ❌    | Does not contain the token `running`. | [Stemming](/reference/indexing/create-index)    |
| Sleke ruining shoez | `sleke` `ruining` `shoez` | ❌    | Does not contain the token `running`. | [Fuzzy](/reference/full-text/fuzzy)             |
...
stmt = (
    select(MockItem.description, MockItem.rating, MockItem.category)
    .where(search.term(MockItem.description, ["shoes", "running"]))
)
