...
canonical: https://www.paradedb.com/docs/reference/full-text/top-k
---

<Note>
  This section covers sorting by a field. To sort by BM25 relevance score, see
  [BM25 Scoring](/reference/full-text/score).
</Note>
...
If a text field is present in the `ORDER BY` clause, it must be indexed with the [literal](/reference/tokenizers/available-tokenizers/literal) or
[literal normalized](/reference/tokenizers/available-tokenizers/literal-normalized) tokenizer.
...
with either the literal or literal normalized tokenizer. See [indexing expressions](/reference/indexing/indexing-expressions) for more information.

```sql
CREATE INDEX search_idx ON mock_items
USING paradedb (id, (lower(description)::pdb.literal))
WITH (key_field='id');
```
...
Range columns are always available to ParadeDB's columnar execution path, so no extra configuration is needed. If a range column appears in a later `ORDER BY` position, the whole `ORDER BY` falls back to a regular Postgres sort. Tiebreaker columns after a leading range key are fine.
...
USING paradedb (id, description, (((metadata->>'weight')::int)::pdb.alias('weight')))
