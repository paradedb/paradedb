...
canonical: https://www.paradedb.com/docs/reference/hybrid/rrf
---

<Note>
  This section uses [vector search](/reference/vector/querying), a beta feature
  available in versions `0.25.0` and above. See [How Hybrid Search
  Works](/reference/hybrid/overview) for background on why results are merged by
...
The two `WHERE` clauses differ because each branch retrieves by its own method. `description ||| 'running shoes'` is the text branch's query. The vector branch's query is the query vector in its `ORDER BY`, so its `WHERE` is [`pdb.all()`](/reference/vector/querying), meaning every row is eligible. To narrow either branch, add [filters](/reference/filtering/overview) to both, so that each is drawing from the same set of eligible rows.
...
All tiebreaker columns must be in the ParadeDB index to keep the Top K optimization. See [Deterministic Sorting](/reference/full-text/score#deterministic-sorting).
