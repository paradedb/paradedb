...
To accelerate filtered queries via segment pruning or enable worker-local parallel joins via range co-partitioning, you can configure the `partition_by` option:
...
See the [Partitioning Guide](/reference/indexing/partition-by) for column selection guidance, data type requirements, and co-partitioned join optimization.
