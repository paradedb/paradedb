...
canonical: https://www.paradedb.com/docs/reference/indexing/indexing-composite
---
...
USING paradedb (id, (ROW(description, category, rating)::item_fields))
WITH (key_field='id');
```
...
Fields in the composite type can use [tokenizers](/reference/tokenizers/overview) and
[token filters](/reference/token-filters/overview) by specifying them as the field type:
...
CREATE INDEX search_idx ON mock_items
USING paradedb (id, (ROW(description, category, in_stock)::item_fields))
WITH (key_field='id');
```
...
- **Duplicate field names**: Field names must be unique within a single ParadeDB index, across all composite types and regular columns in that index.
