...
When both tables participating in an equi-join have ParadeDB indexes partitioned on the join keys using [`partition_by`](/reference/indexing/partition-by), range co-partitioning eliminates cross-worker data exchange entirely:
...
3. Physical over-partitioning: Because real-world tables almost never share identical key distributions or row counts, partition boundaries chosen during independent index builds will not naturally align. To allow the planner to align partitions effectively, we recommend [physically over-partitioning](/reference/indexing/partition-by#physical-over-partitioning) both tables by setting `target_segment_count` to 2–8X your CPU core count (e.g. 16 to 64 segments).
4. Partition pruning: Partitions with non-intersecting ranges between the two tables are eliminated without scanning.

See the [Partitioning Guide](/reference/indexing/partition-by) for full setup instructions, syntax, and column selection advice.
