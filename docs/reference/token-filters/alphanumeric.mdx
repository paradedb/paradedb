...
canonical: https://www.paradedb.com/docs/reference/token-filters/alphanumeric
---
...
(i.e. `0` to `9`). It is supported for all tokenizers besides the [literal](/reference/tokenizers/available-tokenizers/literal) tokenizer.

To enable, append `alpha_num_only=true` to the tokenizer's arguments.

```sql
CREATE INDEX search_idx ON mock_items
USING paradedb (id, (description::pdb.simple('alpha_num_only=true')))
WITH (key_field='id');
```
