...
canonical: https://www.paradedb.com/docs/reference/token-filters/stopwords
---
...
All tokenizers besides the [literal](/reference/tokenizers/available-tokenizers/literal) tokenizer can be configured to automatically remove stopwords
...
```sql
CREATE INDEX search_idx ON mock_items
USING paradedb (id, (description::pdb.simple('stopwords_language=English,French')))
WITH (key_field='id');
```
