...
canonical: https://www.paradedb.com/docs/reference/token-filters/token-length
---
...
USING paradedb (id, (description::pdb.simple('remove_long=100')))
WITH (key_field='id');
```
...
```sql
CREATE INDEX search_idx ON mock_items
USING paradedb (id, (description::pdb.simple('remove_short=3')))
WITH (key_field='id');
```

All tokenizers besides the [literal](/reference/tokenizers/available-tokenizers/literal) tokenizer accept these configurations.
