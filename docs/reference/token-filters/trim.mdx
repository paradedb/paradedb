...
This filter is useful for tokenizers that don't already split on whitespace, like the [literal normalized](/reference/tokenizers/available-tokenizers/literal-normalized)
tokenizer or certain language-specific tokenizers.

```sql
CREATE INDEX search_idx ON mock_items
USING paradedb (id, (description::pdb.literal_normalized('trim=true')))
WITH (key_field='id');
```
