...
canonical: https://www.paradedb.com/docs/reference/token-filters/trim
---
...
This filter is useful for tokenizers that don't already split on whitespace, like the [keyword](/reference/tokenizers/available-tokenizers/keyword)
tokenizer or certain language-specific tokenizers.

```sql
CREATE INDEX search_idx ON mock_items
USING paradedb (id, (description::pdb.keyword('trim=true')))
WITH (key_field='id');
```
...
```sql
SELECT
  '    token with whitespace   '::pdb.keyword::text[],
  '    token with whitespace   '::pdb.keyword('trim=true')::text[];
```
