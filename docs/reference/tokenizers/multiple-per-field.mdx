...
canonical: https://www.paradedb.com/docs/reference/tokenizers/multiple-per-field
---

In many cases, a text field needs to be tokenized multiple ways. For instance, using the [unicode](/reference/tokenizers/available-tokenizers/unicode)
tokenizer for search, and the [literal](/reference/tokenizers/available-tokenizers/literal) tokenizer for [Top K ordering](/reference/full-text/top-k).
...
<Note>
If a text field uses multiple tokenizers and one of them is [literal](/reference/tokenizers/available-tokenizers/literal), we recommend aliasing
