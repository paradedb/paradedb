---
title: Query-Time Tokenizer
description: Use a different tokenizer at search time than at index time
canonical: https://www.paradedb.com/docs/reference/tokenizers/search-tokenizer
---
...
Any [available tokenizer](/reference/tokenizers/overview) can be used as a `search_tokenizer`:
`chinese_compatible`, `edge_ngram`, `icu`, `jieba`, `keyword`, `lindera`, `literal`, `ngram`,
`regex_pattern`, `simple`, `source_code`, `unicode_words`, `whitespace`.
