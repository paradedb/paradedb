...
canonical: https://www.paradedb.com/docs/reference/vector/querying
---

<Note>
  This is a beta feature available in versions `0.25.0` and above. See [How
  Vector Search Works](/reference/vector/overview) for background, and [Indexing
  Vectors](/reference/indexing/indexing-vectors) to set up the index used in
  these examples.
</Note>
...
When one of ParadeDB's [search operators](/reference/filtering/indexed) is present at the same level as the `ORDER BY ... LIMIT`, ParadeDB can accelerate the vector search query.
...
This operator must match the distance metric used by the index. Otherwise, ParadeDB cannot use the index to order results and falls back to a slower brute force sort.

| Operator | Distance      | Index setting       |
| -------- | ------------- | ------------------- |
| `<->`    | L2            | `vector_l2_ops`     |
...
To search within a subset of rows, replace `pdb.all()` with a real predicate using any of the [search operators](/reference/filtering/indexed#non-text-columns).
...
<Note>
  Every column you filter on (e.g. `category` above) must also be part of the
  ParadeDB index. Filters on unindexed columns cannot be evaluated by the
  ParadeDB index, forcing Postgres to recheck them afterward and eliminating the
  benefit of combining vector search with
  [filtering](/reference/filtering/overview).
</Note>
...
If you do not see a `TopKScanExecState`, the query fell back to a less efficient sort. This usually means the distance operator does not match how the vector column was indexed, the query is missing a `LIMIT`, or an `ORDER BY` or filter column is not indexed.
