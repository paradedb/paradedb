...
<Note>
  Recall also depends on the index's `training_sample_ratio` and `max_leaf_size`, set at build time. See
  [Index Options](/reference/indexing/indexing-vectors#index-options).
</Note>
