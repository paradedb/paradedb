...
<Note>
  **Prerequisite** Before performing full text search over a table, it must
  first be [indexed](/search/indexing/bm25).
</Note>

## Overview

Full text search uses the [BM25 index](/search/indexing/bm25) to find rows in a table that match a query.
...
<Note>
  The slop operator is distinct from [fuzzy search](/search/full-text/fuzzy),
  which is used for typo tolerance.
</Note>
...
<Note>
  ...
  [`numeric_fields`](/search/indexing/bm25#creating-a-bm25-index) or
  [`boolean_fields`](/search/indexing/bm25#creating-a-bm25-index).
</Note>
...
<Note>
  ...
  indexed as [`json_fields`](/search/indexing/bm25#creating-a-bm25-index).
</Note>
