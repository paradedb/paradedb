...
```sql
SELECT *, paradedb.highlight_bm25(ctid, '<column_name>')
FROM <table_name>
...
We are working on a release that will enable users to configure `max_num_chars`, or the maximum number
of characters in a highlighted snippet of text.
