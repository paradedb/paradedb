...
To calculate a row's hybrid score, ParadeDB introduces a `rank_hybrid` function. Under the hood,
...
[Advanced search functions](search/full-text/complex) can be passed directly into `bm25_query`:
