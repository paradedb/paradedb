...
client.search(
    table_name,
    Search().query("term", question="Who am I?")
)
```
...
client.search(
    table_name,
    Search().query("fuzzy", question={"value": "Whom I?", "fuzziness": 2})
)
```
...
client.search(
    ...
    Search().query(
        Q(
            "bool",
            must=[Q("match", question="Who am I?")],
            filter=[Q("term", answer="Michael")],
            must_not=[Q("term", question="Where are you?")],
            should=[Q("match", question="Whom are you?")]
        )
    )
)
```
