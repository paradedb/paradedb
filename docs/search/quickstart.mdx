...
<Note>
Prior to version 0.7.5, only integer IDs were supported in the `key_field` column.
If your primary key was a composite or a string, you needed to add another column with serial IDs to your table and use that as your `key_field`.
From version 0.7.6 onwards, [ParadeDB supports non-integer IDs](https://github.com/paradedb/paradedb/pull/1174).
