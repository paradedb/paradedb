---
title: Run Queries
sidebarTitle: Run Your First Queries
description: Run your first queries on ParadeDB
canonical: https://www.paradedb.com/docs/start/run-queries
---

Now that your [environment is configured](/start/connect-your-app), select the codetab for your tool and run some queries.
...
`|||` is ParadeDB's custom [match disjunction](/reference/full-text/match#match-disjunction) operator, which means "find me all documents containing
`running OR shoes`.

If we want all documents containing `running AND shoes`, we can use ParadeDB's `&&&` [match conjunction](/reference/full-text/match#match-conjunction) operator.
...
Next, let's add [BM25 scoring](/reference/full-text/score) to the results, which sorts matches by relevance. To do this, we'll use `pdb.score`.
...
In addition to BM25 and vector similarity, ParadeDB is also optimized for quickly sorting by an arbitrary field, also known as a [Top K](/reference/full-text/top-k) query.
...
Finally, let's also [highlight](/reference/full-text/highlight) the relevant portions of the documents that were matched.
...
[Faceted queries](/reference/aggregates/facets) allow a single query to return both the Top K results and an aggregate value,
...
That's it! Next, let's [load your data](/start/load-data) to start running real queries.
