...
If a new version of `pg_search` or `pg_sparse` is available, it will be listed in the [changelog](/changelog).
...
The latest `pg_search` and `pg_sparse` will always be pegged to the same version. `pgvector`, on the other hand, is
...
<Accordion title="Example Usage">
  ```sql
  ALTER EXTENSION pg_search UPDATE TO '0.4.1';
  ```
</Accordion>
