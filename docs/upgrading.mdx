...
If a new version of `pg_search` or `pg_lakehouse` is available, it will be listed in the [changelog](/changelog).
...
The latest `pg_search` and `pg_lakehouse` will always be pegged to the same version. `pgvector`, on the other hand, is
...
ALTER EXTENSION pg_lakehouse UPDATE TO '<version_number>';
```

<Accordion title="Example Usage">
  ```
  sql ALTER EXTENSION pg_search UPDATE TO '0.8.6';
  sql ALTER EXTENSION pg_lakehouse UPDATE TO '0.8.6';
  ```
</Accordion>
