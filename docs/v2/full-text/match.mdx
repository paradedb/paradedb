---
title: Match
description: Match queries are the standard full text query type in ParadeDB.
canonical: https://docs.paradedb.com/v2/full-text/match
---
...
1. Retrieves the tokenizer configuration of the `description` column. In this example,
   let's assume `description` uses the [simple](/v2/tokenizers/available-tokenizers/simple) tokenizer.
...
| SLeeK RUNNING ShOeS | `sleek` `running` `shoes` | ✅    | Contains both `running` and `shoes`.    | [Lowercasing](/v2/indexing/create-index)                   |
| Sleek run shoe      | `sleek` `run` `shoe`      | ❌    | Contains neither `running` nor `shoes`. | [Stemming](/v2/indexing/create-index)                      |
| Sleke ruining shoez | `sleke` `ruining` `shoez` | ❌    | Contains neither `running` nor `shoes`. | [Fuzzy](/v2/full-text/fuzzy)                               |
...
| Sleke ruining shoez | `sleke` `ruining` `shoez` | ❌    | Does not contain both `running` and `shoes`. | [Fuzzy](/v2/full-text/fuzzy)                               |
