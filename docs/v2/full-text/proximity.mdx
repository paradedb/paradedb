...
<Note>
  Like the [term](/v2/full-text/term) query, the query string in a proximity
  query is treated as a finalized token.
</Note>

<Note>
  Order does not matter for proximity queries -- the term on the left-hand side
  may appear before or after the term on the right-hand side.
</Note>
...
By default, `pdb.prox_regex` will expand to the first `50` regex matches in each document. This limit can be overriden
