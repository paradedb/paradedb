...
<Note>
  All tokenizers besides the exact tokenizer
  [lowercase](/v2/token_filters/lowercase) tokens by default. Make sure to
  account for this when searching for a term.
</Note>

<Note>
  If you are using `===` to do an exact string match on the original text, make
  sure that the text uses the [exact](/v2/tokenizers/available_tokenizers/exact)
  tokenizer.
</Note>
