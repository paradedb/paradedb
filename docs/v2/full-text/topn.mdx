...
1. All `ORDER BY` fields must be indexed. If they are text fields, they [must use the exact tokenizer](#sorting-by-text).
...
If a text field is present in the `ORDER BY` clause, it must be indexed with the [exact](/v2/tokenizers/available_tokenizers/exact) tokenizer.
The reason is that the exact tokenizer preserves the original text, which is necessary for accurate sorting.

Sorting by lowercase text using `lower(<text_field>)` is also supported. To enable this, first ensure that `lower(<text_field>)` is indexed with the exact tokenizer.
