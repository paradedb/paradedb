...
description: The alpha numeric only filter removes any tokens that contain characters that are not ASCII letters.
canonical: https://docs.paradedb.com/v2/token-filters/alphanumeric
---
...
(i.e. `0` to `9`). It is supported for all tokenizers besides the [literal](/v2/tokenizers/available-tokenizers/exact) tokenizer.
