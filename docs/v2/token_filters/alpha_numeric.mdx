...
(i.e. `0` to `9`). It is supported for all tokenizers besides the [exact](/v2/tokenizers/available_tokenizers/exact) tokenizer.
...
```sql
SELECT
  'Tokenize me! 123'::pdb.whitespace::text[],
  'Tokenize me! 123'::pdb.whitespace('alpha_num_only=true')::text[];
```

```ini Expected Response
        text        |      text
--------------------+----------------
 {tokenize,me!,123} | {tokenize,123}
(1 row)
```
