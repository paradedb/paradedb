...
The token length filter automatically removes tokens that are above or below a certain threshold in bytes.
To remove all tokens longer than a threshold, append a `remove_long` configuration to the tokenizer:
...
To remove all tokens shorter than a threshold, use `remove_short`:
...
All tokenizers besides the [exact](/v2/tokenizers/exact) tokenizer accept these configurations.
