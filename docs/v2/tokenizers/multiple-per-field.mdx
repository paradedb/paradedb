...
description: How to tokenize the same text field multiple ways.
canonical: https://docs.paradedb.com/v2/tokenizers/multiple-per-field
---

In many cases, a text field needs to be tokenized multiple ways. For instance, using the [simple](/v2/tokenizers/available-tokenizers/simple)
tokenizer for search, and the [literal](/v2/tokenizers/available-tokenizers/exact) tokenizer for [Top N ordering](/v2/sorting/topn).
...
<Note>
If a text field uses multiple tokenizers and one of them is [literal](/v2/tokenizers/available-tokenizers/exact), we recommend aliasing
