...
ParadeDB introduces a custom index called the _ParadeDB index_.
...
The ParadeDB index is laid out as an [LSM tree](#lsm-tree), where each segment in the tree contains an inverted index, a columnar index, and, when vector fields are indexed, a vector index.
The per-segment indexes optimize for fast reads, while the LSM tree optimizes for high-frequency writes.
...
Segments can also hold a vector index for any [indexed vector column](/documentation/indexing/indexing-vectors). Vectors are clustered with k-means and stored in a SPANN-style
structure, which is similar to an IVF index but keeps per-cluster bounds that let the scan skip clusters that provably cannot improve the current top-K.

Because vectors sit in the same segments as the inverted and columnar indexes, filtered vector search can evaluate similarity and indexed predicates in a single index scan instead
of intersecting results from separate ParadeDB and pgvector indexes. [Hybrid search](/documentation/hybrid/overview) currently runs BM25 and vector branches against the same
ParadeDB index and then fuses their rankings with [reciprocal rank fusion](/documentation/hybrid/rrf).

In the current beta, inserts and updates are immediately searchable, but the vector cluster layout is not maintained incrementally and search quality can degrade under
update-heavy workloads. SPFresh-based incremental maintenance is [in progress](/welcome/roadmap#vector-search).
...
In ParadeDB, every `INSERT`/`UPDATE`/`COPY` statement creates a new segment. Each segment has its own inverted and columnar indexes and, when applicable, vector indexes. The
ParadeDB index is therefore a collection of per-segment structures that can rapidly filter and rank matches before merging the results.
...
`pg_search` also depends on [pgvector](https://github.com/pgvector/pgvector) for its Postgres vector types. ParadeDB indexes those types in its own SPANN-style structure rather
than using pgvector's HNSW or IVF indexes.
