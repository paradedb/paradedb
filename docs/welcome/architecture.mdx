...
In ParadeDB these structures are referred to as [fast fields](/documentation/indexing/fast-fields).
...
ParadeDB introduces several new text search operators to Postgres. For example, `|||` is used for [match disjunction](/v2/full-text/match) queries, whereas `###`
is for [phrase](/v2/full-text/phrase) queries.
...
For queries that need to read large amounts of data like [Top N](/v2/sorting/topn) or aggregate queries, the custom scan automatically spawns additional workers to execute the query
...
<Note>
  ...
  workers](/documentation/configuration/read-throughput).
</Note>
