...
row-oriented layout. All text fields which use the [literal](/documentation/tokenizers/available-tokenizers/literal) or [normalized](/documentation/tokenizers/available-tokenizers/normalized) tokenizer, or are non-text,
...
For queries that need to read large amounts of data like [Top N](/documentation/sorting/topn) or aggregate queries, the custom scan automatically spawns additional workers to execute the query
