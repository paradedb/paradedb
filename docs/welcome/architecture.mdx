...
Because the vectors sit in the same segment as the inverted and columnar indexes, a query that combines vector similarity with text predicates and filters resolves against one
index, rather than intersecting the results of two separate ones. This is what makes [hybrid search](/documentation/hybrid/overview) and filtered vector search efficient.
