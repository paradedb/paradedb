...
All of the functionalities of ParadeDB are shipped as a Postgres extension called `pg_search`.

## Index Access Method

![ParadeDB BYOC Topology](/images/architecture_indexam.png)

Postgre supports pluggable index access methods, which allow extensions to define their own index types. Indexes are useful because they allow the
underlying Postgres table (which Postgres calls a *heap table*) to be represented in alternative data structures that are optimized for different types
of queries. For instance, the built-in Postgres B-tree index is used for point lookups and range scans.

ParadeDB introduces a custom access method called the BM25 index that's designed full text search and analytics.
