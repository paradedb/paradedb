---
title: Guarantees
description: An overview of the guarantees that ParadeDB provides.
...
by configuring the [`max_parallel_worker` pool](/documentation/configuration/read-throughput#raise-parallel-workers) and [buffer cache size](/documentation/configuration/read-throughput#raise-shared-buffers),
whereas writes can be improved by increasing [per-statement memory](/documentation/configuration/write-throughput#increase-memory-for-bulk-updates).
