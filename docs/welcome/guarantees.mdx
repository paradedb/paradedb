...
by configuring the [`max_parallel_worker` pool](/documentation/configuration/read_throughput#raise-parallel-workers) and [buffer cache size](/documentation/configuration/read_throughput#raise-shared-buffers),
whereas writes can be improved by increasing [per-statement memory](/documentation/configuration/write_throughput#increase-memory-for-bulk-updates).
