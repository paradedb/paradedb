...
ParadeDB is an open source, ACID-compliant alternative to Elasticsearch, designed for search and analytics workloads. The product is built on Postgres as a Postgres extension, not a fork of Postgres.
...
| **Analytics features**      | Not an analytical DB (no column store, batch processing, etc.)           | Column store, batch processing, parallelization via sharding                                           | Column store, batch processing, parallelization via Postgres [parallel workers](/documentation/configuration/parallel) |
