...
full-text and semantic search in one query, with no separate vector database to keep in sync. Vectors run on pgvector today, and a
high-performance, pgvector-compatible version is coming to the ParadeDB index soon.
...
Lucene. It handles full-text ranking and aggregations together and stays current inside your transactions, so search never lags your
writes. It goes toe-to-toe with dedicated search engines on full-text performance, often coming out on top. Vector search is coming to the
same index soon, so hybrid queries will run on one index too. [See how it's built →](/welcome/architecture)
