...
- **Only pgvector's `vector` type is indexable.** The `halfvec`, `sparsevec`, and `bit` types are not yet supported, and neither is scalar or binary quantization. Vectors are stored at full precision, so a large index needs proportionally more memory and disk.
- **Search is approximate.** Queries probe a subset of the index's clusters rather than every vector, so recall is traded against latency. See [Tuning Recall and Latency](/documentation/vector/tuning).
- **Incremental index maintenance is not yet available.** Inserts and updates are transactionally indexed and immediately searchable, but vector search quality can degrade over time under update-heavy workloads.
