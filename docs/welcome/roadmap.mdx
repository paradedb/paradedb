...
- **Quantization**. Scalar (`int8`) and binary quantization will reduce the memory and I/O cost of large vector indexes.
- **`halfvec` support**. Support pgvector's half-precision `halfvec` (fp16) type in addition to the `vector` type that is indexed today.
