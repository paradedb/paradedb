...
- **DataFusion-powered aggregates**. Migrate the aggregate scan from Tantivy to Apache DataFusion to broaden SQL aggregate coverage and improve performance.

### Vector Search Improvements

- Improve vector search in Postgres, specifically for queries that combine vector similarity with metadata filtering and full-text search predicates.
