...
use super::table::{
    RESERVED_CMAX_FIELD, RESERVED_CMIN_FIELD, RESERVED_TID_FIELD, RESERVED_XMAX_FIELD,
    RESERVED_XMIN_FIELD,
};
...
pub trait PostgresBatch {
    ...
    fn remove_cmin_column(&mut self) -> Result<Arc<dyn Array>, ArrowError>;
    fn remove_cmax_column(&mut self) -> Result<Arc<dyn Array>, ArrowError>;
}
...
impl PostgresBatch for RecordBatch {
    ...
    fn remove_cmin_column(&mut self) -> Result<Arc<dyn Array>, ArrowError> {
        let schema = self.schema();
        let index = schema.index_of(RESERVED_CMIN_FIELD)?;
    ...
    fn remove_cmax_column(&mut self) -> Result<Arc<dyn Array>, ArrowError> {
        let schema = self.schema();
        let index = schema.index_of(RESERVED_CMAX_FIELD)?;
