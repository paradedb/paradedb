...
use super::table::{RESERVED_TID_FIELD, RESERVED_XMIN_FIELD};

pub trait PostgresBatch {
    fn remove_tid_column(&mut self) -> Result<Arc<dyn Array>, RecordBatchError>;
    fn remove_xmin_column(&mut self) -> Result<Arc<dyn Array>, RecordBatchError>;
}
...
impl PostgresBatch for RecordBatch {
    ...
    fn remove_xmin_column(&mut self) -> Result<Arc<dyn Array>, RecordBatchError> {
        let schema = self.schema();
        let tid_index = schema.index_of(RESERVED_XMIN_FIELD)?;
