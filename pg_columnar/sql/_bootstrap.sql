...
CREATE OR REPLACE PROCEDURE paradedb.create_test_table()
LANGUAGE plpgsql
AS $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_tables
                   WHERE schemaname = 'paradedb' AND tablename = 'test_table') THEN
        CREATE TABLE paradedb.test_table (
        ...
        INSERT INTO paradedb.test_table (description, rating, category, in_stock, metadata)
        VALUES
    ...
    ELSE
        RAISE WARNING 'The table paradedb.test_table already exists, skipping.';
    END IF;
END $$;