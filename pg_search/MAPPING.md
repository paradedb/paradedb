...
| Query entry / exact fallback | Tantivy | `../tantivy/src/vector/backend.rs::VectorBackend::top_n_by` branches once on `VectorIndexReader::index()`: `None` uses `exact_top_n`; `Some` uses `approximate_top_n`. A no-slot or `max_scan_levels = 0` exact path belongs at this dispatch boundary. |
...
| Current vector GUCs | pg_search | `pg_search/src/gucs.rs` owns the IVF probe controls consumed by `pg_search/src/index/reader/index.rs`; `max_scan_levels` must be added here and passed into Tantivy's per-query vector context. |
