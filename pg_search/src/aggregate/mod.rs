...
pub fn execute_aggregate(
    ...
    unsafe {
        ...
        if let Some(mut process) = launch_parallel_process!(
            ...
            let reuse_reader = gucs::enable_count_reader_reuse()
                && solve_mvcc
    ...
    impl<C: Collector> Collector for MVCCFilterCollector<C> {
        ...
        fn for_segment(
            ...
            Ok(MVCCFilterSegmentCollector {
                inner,
                ctid_ff: if lock.is_some() {
                    FFType::new(segment.fast_fields(), CTID_FIELD_NAME)
                } else {
                    FFType::Junk
                },
                lock,
                ...
                ctids_buffer: Vec::with_capacity(capacity),
