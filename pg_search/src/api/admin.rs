...
// These `pdb.*` admin functions are implemented below in the Rust `pdb` module, but
// they are not emitted into the fresh-install SQL generated by `cargo pgrx package`.
// Keep them in the install script explicitly so new installs and generated release
// schemas match the supported runtime surface area.
extension_sql!(
    r#"
            CREATE SCHEMA IF NOT EXISTS pdb;

            CREATE OR REPLACE FUNCTION pdb."indexes"() RETURNS TABLE (
                "schemaname" TEXT,
                "tablename" TEXT,
                "indexname" TEXT,
                "indexrelid" oid,
            ...
            CREATE OR REPLACE FUNCTION pdb."index_segments"(
                "index" regclass
            ) RETURNS TABLE (
                "partition_name" TEXT,
                "segment_idx" INT,
            ...
            CREATE OR REPLACE FUNCTION pdb."verify_all_indexes"(
            ...
            CREATE OR REPLACE FUNCTION pdb."verify_index"(
                "index" regclass,
                "heapallindexed" bool DEFAULT false,
                "sample_rate" double precision DEFAULT NULL,
                "report_progress" bool DEFAULT false,
...
"#,
    name = "pdb_admin_fresh_install_functions"
);
