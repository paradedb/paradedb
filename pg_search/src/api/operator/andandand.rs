...
/// `match_conjunction_array` (both carry `conjunction_mode = Some(true)`) with any
/// `fuzzy_data` / `slop_data` re-applied. Shared with `search_with_match_conjunction_support`'s
...
/// `Match` with `conjunction_mode = Some(true)` first. Fixes #5779 for `&&&`.
