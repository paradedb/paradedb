    ...
    unsafe {
        ...
        if let Some((srs, jv_info, inner_lhs, rhs)) = try_jsonb_values_expansion(node) {
            if nodecast!(Const, T_Const, rhs).is_some() {
                return expand_jsonb_values_match_query_const(srs, &jv_info, inner_lhs, rhs);
            }
            return expand_jsonb_values_match_query_exec(srs, &jv_info, inner_lhs, rhs);
        }
...
unsafe fn expand_jsonb_values_match_query_const(
    ...
    let const_node = nodecast!(Const, T_Const, rhs)
        .expect("expand_jsonb_values_match_query_const requires a Const RHS");
    ...
    let base_query: pdb::Query = match (*const_node).consttype {
        ...
        pg_sys::TEXTARRAYOID | pg_sys::VARCHARARRAYOID => {
            ...
            match_disjunction_array(tokens)
        }
...
unsafe fn expand_jsonb_values_match_query_exec(
    srs: *mut pg_sys::SupportRequestSimplify,
    jv_info: &crate::api::operator::JsonbValuesInfo,
    lhs: *mut pg_sys::Node,
    rhs: *mut pg_sys::Node,
