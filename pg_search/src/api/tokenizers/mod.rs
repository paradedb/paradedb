...
pub fn search_field_config_from_type(
    ...
    // if it is text or JSON, it also default to true for literal and keyword
    ...
    let (fast, fieldnorms, record) = if matches!(
        type_name.as_str(),
        "literal" | "keyword" | "literal_normalized"
    ) {
        // literal and keyword default to fast=true (columnar=true)
        let fast = columnar_explicit.unwrap_or(true);

        // literal and keyword default to fieldnorms=false
