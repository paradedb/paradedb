...
/// unchanged, and sweep-based tuning self-calibrates). On REPLICATED
/// indexes a fixed fraction now buys less candidate volume than the old
/// count budget did — by design, because replicas no longer inflate
/// spend; retune the fraction upward once (≈ ×replication factor for
/// matched recall). Default 0.02. `1.0` scans everything. Allocation is
/// per segment — f of the index's work, spent where the work is.
/// Segments without stored radii meter plain cluster counts (legacy
/// regime).
