...
pub fn init() {
    ...
    GucRegistry::define_bool_guc(
        ...
        c"When enabled, DataFusion optimizer rules co-partition inner joins across tables on the boundaries their partitioned builds recorded. Both tables must define partition_by on the join key, and at least one must have been built over its rows. Default is false.",
