...
pub fn index_memory_segment(
    ...
    let num_docs = segment_meta.num_docs();
    ...
    unsafe {
        ...
        mutable_segment.for_each(|_, entry| {
            if entry.deleted || count >= num_docs {
                // TODO: This could mean that deletes take the doc count to 0. Should likely handle
                // deletes by indexing a placeholder and then deleting it.
                return;
            }
            count += 1;
            ...
            if fetched {
                ...
                // TODO: Because we used `SnapshotAny`, all tuples should be visible to us (which
                // haven't been deleted by `ambulkdelete`), but for some reason we are still
                // unable to fetch some tuples in update-heavy workloads.
                //
                // Try to better understand this case before merging, but for now: index a
                // ctid-only doc.
                writer
                    .insert(tantivy::TantivyDocument::new(), entry.ctid, || {
                        unreachable!("No limits configured: should not finalize.")
                    })
                    .expect("TODO: Failed to index heap tuple.");
            }
    ...
    let actual_segment = writer.finalize_nocommit()?.expect(
        "Segment should be non-empty because we created a `SegmentMeta` for it in the first place.",
    );

    assert_eq!(actual_segment.max_doc(), num_docs);
