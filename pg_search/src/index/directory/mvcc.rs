        ...
        match meta_entry {
            ...
            LoadedSegmentMetaEntry::Memory {
                ...
                let file_handle = directory
                    .get_or_init(|| {
                        index_memory_segment(&self.indexrel, &tantivy_meta, &meta)
    ...
    for ctid in ctids {
        ...
        unsafe {
            ...
            if fetched {
                ...
                // Creating a Document clones all necessary heap data, so we can release the buffer
                // before indexing.
                ...
                // SnapshotAny setting).
                writer
                    .insert(tantivy::TantivyDocument::new(), ctid, || {
                        unreachable!("No limits configured: should not finalize.")
                    })
                    .expect("TODO: Failed to index heap tuple.");
            }
