...
impl SearchIndexReader {
    ...
    /// the base query weight, so scored text preparation runs only once.
...
mod tests {
    ...
    fn dispatched_statistics_are_opened_once_for_the_frozen_view() {
        ...
        // Assigning execution segments and proving predicates share one set of opened
        // components. Planning chooses split-point values separately.
        crate::index::stats::segments_for_partition(
            &leader,
            &RangePartitioning {
    ...
    fn partition_queries_share_preparation_without_changing_scores() {
        ...
        for query in [
            ...
            for scoring in [false, true] {
                // Included only, partial only, both, and no selected segments.
                    ...
                    [(11, 0, 1, 0), (5, 0, 0, 1), (15, 0, 1, 1), (100, 1, 0, 0)]
                {
                    ...
                    let scan = reader.search_segments_with_range_filter(
                        segments.included.into_iter(),
                        &bounds,
                        segments.partially_included.into_iter(),
                    );
    ...
    fn partition_filter_is_omitted_only_when_redundant() {
        ...
        // NULL belongs to the first partition. It can cover this whole segment;
        // a split inside the non-NULL values must retain the exact partition query.
        for (split, partition, expected, covered) in
            [(21, 0, 3, true), (15, 0, 2, false), (15, 1, 1, false)]
        {
            let partitioning = RangePartitioning {
            ...
            check_filter(
                &reader,
                &partitioning.partition_bounds(partition),
                &partitioning,
                partition,
                expected,
                covered,
