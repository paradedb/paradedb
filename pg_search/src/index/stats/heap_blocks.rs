...
use crate::api::TID_BLOCK_FIELD_NAME;
...
pub(super) fn write(segment: &Segment, out: &mut CompositeWrite) -> tantivy::Result<()> {
    ...
    let Ok(field) = schema.get_field(TID_BLOCK_FIELD_NAME) else {
        return Ok(());
    };
    let fast = ColumnarReader::open(segment.open_read(SegmentComponent::FastFields)?)?;
    let handles = fast.read_columns(TID_BLOCK_FIELD_NAME)?;
    ...
    for (doc, value) in column.values.iter().enumerate() {
        ...
        let block = u32::try_from(value).map_err(|_| invalid())?;
