...
impl BuildState {
    fn new(
        ...
        mut search_index: SearchIndex,
    ) -> Self {
        let writer = search_index
            .get_writer(WriterResources::CreateIndex, index_options)
            .expect("should be able to create SearchIndexWriter");
        BuildState {
            count: 0,
...
pub extern "C" fn ambuild(
    ...
    let search_index = SearchIndex::create_index(index_oid, fields, key_field_index)
        .expect("error creating new index instance");
    let mut state = BuildState::new(
        &index_relation,
        index_info,
        unsafe { &*index_relation.rd_options.cast::<SearchIndexCreateOptions>() },
        search_index,
    );
    ...
    state.writer.commit().expect("index commit should succeed");
    // TODO:  how to handle waiting for merging threads?
    // state.writer
    //     .underlying_writer
...
unsafe extern "C" fn build_callback(
    ...
    let search_index = &(*build_state).index;
