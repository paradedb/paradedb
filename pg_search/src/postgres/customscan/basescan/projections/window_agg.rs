...
//! Window Function Support for TopK Queries (Faceting)
//!
//! This module implements window function support for TopK queries, enabling
...
//! 4. **Execution**: Execute aggregations via Tantivy's `MultiCollector` in TopK scan
...
//! - **Single Scan**: Allows TopK + aggregation in one scan pass
//!
...
pub mod window_aggregates {
    /// Only allow window function replacement in TopK queries (with ORDER BY and LIMIT).
    /// When true, window functions are only replaced with window_agg in TopK execution context.
...
/// Information about a window aggregate to compute during TopK execution
...
pub unsafe fn extract_and_convert_window_functions(
    ...
    // Check TopK context requirement if enabled
    if window_aggregates::ONLY_ALLOW_TOP_K {
        ...
        if !is_top_k_query {
            // Not a TopK query - return empty map so PostgreSQL handles all window functions
            return HashMap::new();
        }
    }
