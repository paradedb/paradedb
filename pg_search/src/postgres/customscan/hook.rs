...
use crate::postgres::customscan::pdbscan::privdat::{
    CompositeSide, JoinCompositeInfo, PrivateData,
};
...
/// Check if this join step involves composite relations and determine if we can handle it
unsafe fn analyze_composite_relations(
    outerrel: *mut pg_sys::RelOptInfo,
    innerrel: *mut pg_sys::RelOptInfo,
) -> CompositeRelationAnalysis {
    ...
    match (outer_relid_count > 1, inner_relid_count > 1) {
        (false, false) => {
            // Both sides are base relations - we can handle this
            CompositeRelationAnalysis::BothBaseRelations
        }
        (true, false) => {
            // Outer is composite, inner is base - we can potentially handle this
            CompositeRelationAnalysis::OuterComposite
        }
        (false, true) => {
            // Outer is base, inner is composite - we can potentially handle this
            CompositeRelationAnalysis::InnerComposite
        }
        (true, true) => {
            // Both sides are composite - too complex for now
            CompositeRelationAnalysis::BothComposite
        }
    }
}

/// Analysis result for composite relations
#[derive(Debug, Clone, PartialEq)]
enum CompositeRelationAnalysis {
    /// Both sides are base relations (single relation each)
    BothBaseRelations,
    /// Outer side is composite (multiple relations), inner is base
    OuterComposite,
    /// Outer side is base, inner side is composite (multiple relations)
    InnerComposite,
    /// Both sides are composite (too complex to handle)
    BothComposite,
}
...
pub extern "C-unwind" fn paradedb_join_pathlist_callback<CS: CustomScan>(
    ...
    unsafe {
        ...
        // Analyze the composite relation structure
        let composite_analysis = analyze_composite_relations(outerrel, innerrel);

        match composite_analysis {
            CompositeRelationAnalysis::BothBaseRelations => {
        ...
        // Enhanced feasibility check based on composite relation analysis
        let (outer_has_bm25, inner_has_bm25) = match composite_analysis {
            CompositeRelationAnalysis::BothBaseRelations => {
                // Standard case: check both sides for BM25 indexes
                let outer_has_bm25 = has_any_bm25_index(root, outerrel);
                let inner_has_bm25 = has_any_bm25_index(root, innerrel);
                (outer_has_bm25, inner_has_bm25)
            }
            CompositeRelationAnalysis::OuterComposite => {
                // Outer is composite, inner is base - only check inner for BM25
                let inner_has_bm25 = has_any_bm25_index(root, innerrel);
                warning!(
                    "ParadeDB: Composite join - outer is composite, checking inner for BM25: {}",
                    inner_has_bm25
                );
                (false, inner_has_bm25) // We don't need BM25 on composite side
            }
            CompositeRelationAnalysis::InnerComposite => {
                // Outer is base, inner is composite - only check outer for BM25
                let outer_has_bm25 = has_any_bm25_index(root, outerrel);
                warning!(
                    "ParadeDB: Composite join - inner is composite, checking outer for BM25: {}",
                    outer_has_bm25
                );
                (outer_has_bm25, false) // We don't need BM25 on composite side
            }
            CompositeRelationAnalysis::BothComposite => {
        ...
        // For composite joins, we only need BM25 on the base relation side
        let needs_bm25 = match composite_analysis {
            CompositeRelationAnalysis::BothBaseRelations => outer_has_bm25 || inner_has_bm25,
            CompositeRelationAnalysis::OuterComposite => inner_has_bm25,
            CompositeRelationAnalysis::InnerComposite => outer_has_bm25,
            CompositeRelationAnalysis::BothComposite => false,
        };
        ...
        if let Some(ref predicates) = search_predicates {
            ...
            // Final feasibility check based on composite relation analysis
            let join_feasible = match composite_analysis {
                CompositeRelationAnalysis::BothBaseRelations => {
                    // Standard case: need at least one side with both BM25 and search
                    let outer_feasible = outer_has_bm25 && outer_has_search;
                    let inner_feasible = inner_has_bm25 && inner_has_search;
                    outer_feasible || inner_feasible
                }
                CompositeRelationAnalysis::OuterComposite => {
                    // Outer is composite, inner is base - need inner to have BM25 and search
                    inner_has_bm25 && inner_has_search
                }
                CompositeRelationAnalysis::InnerComposite => {
                    // Inner is composite, outer is base - need outer to have BM25 and search
                    outer_has_bm25 && outer_has_search
                }
                CompositeRelationAnalysis::BothComposite => false, // Already rejected
            };

            if !join_feasible {
                warning!(
                    "ParadeDB: Skipping join - feasibility check failed for composite analysis {:?} (outer: BM25={}, search={}; inner: BM25={}, search={})",
                    composite_analysis,
                    outer_has_bm25,
                    outer_has_search,
                    inner_has_bm25,
            ...
            warning!(
                "ParadeDB: Join is feasible based on composite analysis: {:?}",
                composite_analysis
            );
...
unsafe fn create_search_join_path<CS: CustomScan>(
    ...
    composite_analysis: &CompositeRelationAnalysis,
    ...
    let composite_info = match composite_analysis {
        ...
        CompositeRelationAnalysis::OuterComposite => {
            // Determine if the base side (inner) has search predicates
            let base_has_search = search_predicates
                .map(|p| {
                    p.inner_predicates
                        .iter()
                        .any(|pred| pred.uses_search_operator)
                })
                .unwrap_or(false);
            let composite_has_search = search_predicates
                .map(|p| {
                    p.outer_predicates
                        .iter()
                        .any(|pred| pred.uses_search_operator)
            ...
            Some(JoinCompositeInfo {
                ...
                base_has_search,
                composite_has_search,
            })
        }
        CompositeRelationAnalysis::InnerComposite => {
            // Determine if the base side (outer) has search predicates
            let base_has_search = search_predicates
                .map(|p| {
                    p.outer_predicates
                        .iter()
        ...
        CompositeRelationAnalysis::BothComposite => None, // Already rejected
    };

    if let Some(ref info) = composite_info {
        warning!(
            "ParadeDB: Storing composite join info - composite_side: {:?}, base_has_search: {}, composite_has_search: {}",
            info.composite_side,
            info.base_has_search,
            info.composite_has_search
        );
    }
