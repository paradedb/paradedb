...
1. **[`RangePartitioningRule`](range_partitioning_rule.rs)** — chooses compatible split points from the execution manifests before lowering the logical plan, so range selection and segment pruning reuse the same opened statistics
