    ...
    unsafe fn build_result_tuple(
        ...
        for (i, col_info) in output_columns.iter().enumerate() {
            ...
            if col_info.is_score {
                // Determine which score to use based on which side the score references
                // - If col_info.is_outer == driving_is_outer: score references driving side
                // - Otherwise: score references build side
                let score = if col_info.is_outer == driving_is_outer {
                    driving_score
                } else {
                    build_score
                };
