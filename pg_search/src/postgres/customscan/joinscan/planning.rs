    ...
    'pathkey: for pathkey_ptr in pathkeys.iter_ptr() {
        ...
        for member in members.iter_ptr() {
            ...
            if let Some(rtis) = extract_score_sum(expr.cast()) {
                if rtis.iter().all(|rti| source_rtis.contains(rti)) {
                    continue 'pathkey;
                }
            }
...
pub(super) unsafe fn distinct_collations_are_deterministic(root: *mut pg_sys::PlannerInfo) -> bool {
    ...
    let all_deterministic = distinct_list.iter_ptr().all(|clause| {
        ...
        if score_funcoids().contains(&(*funcexpr).funcid) {
            ...
            if args.len() == 1 {
                if let Some(arg) = args.get_ptr(0) {
                    if let Some(var) = nodecast!(Var, T_Var, strip_wrappers(arg)) {
    ...
    if let Some(opexpr) = nodecast!(OpExpr, T_OpExpr, stripped) {
        ...
        if !opname_ptr.is_null() && !namespace_ptr.is_null() {
            ...
            if schema == b"pg_catalog" && opname == b"+" {
                let args = PgList::<pg_sys::Node>::from_pg((*opexpr).args);
                if args.len() == 2 {
                    if let (Some(left), Some(right)) = (args.get_ptr(0), args.get_ptr(1)) {
