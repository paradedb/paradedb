...
///
/// A frame whose serialized size fits in `slot_capacity - SLOT_HEADER_BYTES` rides
/// the single-slot fast path as `Complete`. Anything larger is fragmented across
/// `n_slots = ceil(frame_len / per_slot_payload)` consecutive ring slots: the
/// first carries `First` with `n_slots` in the flags upper bits, the middle ones
/// `Continue`, the final one `Last`. Producers reserve the run with one
/// `tail.compare_exchange(T, T + n_slots)`, so other producers can never
/// interleave their fragments — the receiver always sees the slots in producer
/// order.
#[repr(u32)]
...
///
/// History:
/// - v1: initial single-slot-per-frame layout. `SlotHeader { seq, len, _pad }`.
/// - v2: multi-slot fragmentation. `SlotHeader { seq, len, flags }` where flags
///   encodes `FragmentKind` + (for First) `n_slots`. Frames larger than
///   `slot_capacity - SLOT_HEADER_BYTES` now span N consecutive slots reserved
...
impl DsmMpscReceiver {
    ...
    /// Drain a single-slot `Complete` frame at `head`. Identical to the v1
    /// receiver path.
    fn recv_single_slot(
        &self,
    ...
    /// Reassemble an `n_slots`-fragment frame starting at `head`. Returns `Empty`
    /// (without advancing `head`) if any continuation slot isn't yet published —
    /// the producer's `try_send_multi_slot` may still be mid-publish. The caller
    /// retries the drain; the First slot stays put with its flags intact.
    fn recv_multi_slot(
        &self,
        header: &DsmMpscRingHeader,
        head: u64,
...
impl DsmMpscSender {
    ...
    /// - `Err(Full)` if no slot run is available right now; caller should yield + retry.
    /// - `Err(Detached)` if the receiver has detached; caller should stop.
    /// - `Err(MessageTooLarge)` if the frame would need more than `ring_size` slots
    ///   (the largest writable frame is `ring_size * (slot_capacity - SLOT_HEADER_BYTES)`).
    ///
    /// Frames at or below the per-slot payload capacity ride a single-slot fast path
    /// identical to the v1 protocol. Larger frames are split across consecutive ring
    /// slots reserved by a single `tail.compare_exchange(T, T + n_slots)`, so other
    /// producers can never interleave their fragments — the receiver always reads
    /// the slots in producer order. See [`FragmentKind`].
    ...
    /// Multi-slot path. Atomically reserve `n_slots` consecutive ring positions by
    /// CAS-advancing `tail` from T to T + n_slots, then publish each fragment in
    /// ascending order with First/Continue/Last flags. The CAS check requires all N
    /// target slots to already be in their expected empty round, which is the
    /// equivalent of v1's per-slot `seq == tail` check generalized to a run.
    ///
    /// Publishing in ascending order is load-bearing: the receiver only starts
    /// reassembling once slot[head] (the First) is ready, then waits for each
    /// subsequent slot in order. Out-of-order publish would make the receiver block
    /// on a Continue slot whose data is already there but whose seq hasn't been
    /// stored yet — wasting one drain pass per producer fragment.
...
mod tests {
    ...
    fn message_too_large_is_rejected() {
        // With multi-slot fragmentation, MessageTooLarge fires only when the frame
        // would need more than `ring_size` slots. Per-slot oversize is no longer
        // rejected — it splits across consecutive slots instead.
    ...
    /// come out of the receiver as a separate, intact byte sequence — fragments do
    /// not leak into the following frame.
    #[test]
