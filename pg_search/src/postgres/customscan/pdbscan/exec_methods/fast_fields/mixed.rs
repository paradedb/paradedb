                ...
                Some((scored, doc_address, field_values)) => {
                    ...
                    if is_visible {
                            ...
                            .enumerate()
                        {
                            ...
                            // Collect any strings and numeric fast fields which gather during the
                            // aggregate.
    ...
    fn next_segment(&mut self) -> bool {
            ...
            string_sort_column.zip(string_sort_results)
        {
            ...
            self.current_segment = Box::new(
                ...
                .map(move |((_, _, doc_addr), term_value)| {
                    // We know that there is at least one other column, so the field values for this
                    // doc should already exist.
                    let (mut field_values, score) = doc_fields.remove(&doc_addr).unwrap();
