...
//! Join execution methods organized into focused modules
//!
...
pub use state::{
    cleanup_join_execution, exec_join_step, init_join_execution, IntermediateResultIterator,
...
// JoinExecState is now defined in the state module

// IntermediateResultIterator is now defined in the state module

// IntermediateSide is now defined in the state module

// JoinExecPhase is now defined in the state module

// JoinTupleSlots and JoinExecStats are now defined in the state module

// Default implementations are now in the state module

// JoinExecState implementation is now in the state module

// init_join_execution is now defined in the state module
...
// exec_join_step is now defined in the state module

// cleanup_join_execution is now defined in the state module
...
unsafe fn fetch_real_join_column_values(
    ...
    if let Some(ref mut join_state) = state.custom_state_mut().join_exec_state {
        join_state.stats.heap_fetch_successes += successful_fetches as u32;
    }
...
pub unsafe fn validate_join_execution_prerequisites(
    state: &mut CustomScanStateWrapper<PdbScan>,
) -> bool {
...
pub unsafe fn init_search_execution_safe(state: &mut CustomScanStateWrapper<PdbScan>) -> bool {
...
unsafe fn execute_outer_search_safe(state: &mut CustomScanStateWrapper<PdbScan>) -> bool {
    if let Some(ref mut join_state) = state.custom_state_mut().join_exec_state {
        let outer_count = join_state
            ...
            .map(|r| r.len())
            .unwrap_or(0);
        join_state.stats.outer_tuples = outer_count as u32;
...
unsafe fn execute_inner_search_safe(state: &mut CustomScanStateWrapper<PdbScan>) -> bool {
    if let Some(ref mut join_state) = state.custom_state_mut().join_exec_state {
        let inner_count = join_state
            ...
            .map(|r| r.len())
            .unwrap_or(0);
        join_state.stats.inner_tuples = inner_count as u32;
...
pub unsafe fn match_and_return_next_tuple_safe(
    state: &mut CustomScanStateWrapper<PdbScan>,
) -> *mut pg_sys::TupleTableSlot {
