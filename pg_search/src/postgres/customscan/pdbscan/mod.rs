...
fn assign_exec_method(builder: &mut CustomScanStateBuilder<PdbScan, PrivateData>) {
    let exec_method_type = builder.custom_state_ref().exec_method_type.clone();
    match exec_method_type.clone() {
        ExecMethodType::Normal => builder
            .custom_state()
            .assign_exec_method(NormalScanExecState::default(), ExecMethodType::Normal),
        ExecMethodType::TopN {
            heaprelid,
            limit,
            sort_direction,
