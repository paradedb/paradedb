...
pub(crate) fn create_placeholder_targetlist(
    ...
    let needs_projection = targetlist_pg.iter_ptr().any(|te| {
        ...
        !is_top_level_placeholder
            && expr_contains_placeholder_funcexpr(te.expr as *mut pg_sys::Node, placeholder_funcid)
    });
...
fn expr_contains_placeholder_funcexpr(
    node: *mut pg_sys::Node,
    placeholder_funcid: pg_sys::Oid,
) -> bool {
    ...
    unsafe { walker(node, addr_of_mut!(ctx).cast()) };
    ctx.found
}
...
pub fn maybe_needs_const_projections(node: *mut pg_sys::Node) -> bool {
    #[pg_guard]
    unsafe extern "C-unwind" fn walker(
        node: *mut pg_sys::Node,
