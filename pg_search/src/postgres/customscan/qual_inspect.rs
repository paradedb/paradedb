    ...
    match (*node).type_ {
        ...
        pg_sys::NodeTag::T_Var => {
            ...
            if let Some(root) = context.planner_info() {
                ...
                Some(Qual::HeapExpr {
                    expr_node: node,
                    expr_desc: deparse_expr_for_index(context, indexrel, node),
    ...
    if pushdown_result.is_none() {
        ...
        if contains_relation_reference(opexpr_node, rti) {
            ...
            Some(Qual::HeapExpr {
                ...
                expr_desc: deparse_expr_for_index(context, indexrel, opexpr_node),
                search_query_input: Box::new(SearchQueryInput::All),
            })
...
unsafe fn create_heap_expr_for_field_ref(
    ...
    if (*var_node).varno as pg_sys::Index == rti {
        ...
        *uses_tantivy_to_query = true;
        ...
        Some(Qual::HeapExpr {
            expr_node,
            expr_desc: deparse_expr_for_index(&context, indexrel, expr_node),
            search_query_input: Box::new(SearchQueryInput::All),
        })
    } else {
        None
