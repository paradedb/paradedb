...
pub unsafe extern "C-unwind" fn aminsert(
    ...
    let result = catch_unwind(|| {
        ...
        match &mut state.mode {
            InsertMode::Mutable(mode) => state.per_row_context.switch_to(|cxt| {
                mode.ctids.push(item_pointer_to_u64(*ctid));
                cxt.reset();
                true
            }),
...
pub unsafe extern "C-unwind" fn aminsertcleanup(
    ...
    match std::mem::replace(&mut state.mode, InsertMode::Completed) {
        InsertMode::Mutable(mode) => aminsertcleanup_mutable(mode, &state.indexrel),
        InsertMode::Completed => {
            panic!("aminsertcleanup was called twice.");
        }
    };
...
#[allow(dead_code)]
fn aminsertcleanup_immutable(mut writer: Option<SerialIndexWriter>) {
...
unsafe fn aminsertcleanup_mutable(mode: InsertModeMutable, indexrel: &PgSearchRelation) {
    let entries = mode
        .ctids
        .into_iter()
        .map(|ctid| MutableSegmentEntry {
            ctid,
            deleted: false,
        })
        .collect::<Vec<_>>();
