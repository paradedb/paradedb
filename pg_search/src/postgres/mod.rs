...
impl ParallelScanState {
    ...
    pub fn aggregation_results(&self) -> Option<IntermediateAggregationResults> {
        self.payload.aggregates().and_then(|agg| {
            let serialized_aggs = &agg.agg_data[..agg.serialized_aggregations_len];
            if !serialized_aggs.is_empty() {
                postcard::from_bytes(serialized_aggs).ok()
            } else {
                None
            }
        })
    ...
    pub fn aggregation_wait(&mut self) -> Option<IntermediateAggregationResults> {
        loop {
            check_for_interrupts!();
            let finalized = {
                let _lock = self.acquire_mutex();
                self.payload.aggregates().as_ref()?.received_count == self.nsegments
            };

            if finalized {
                let _lock = self.acquire_mutex();
                let agg = self.payload.aggregates().unwrap();
                let buffer = &agg.agg_data[agg.serialized_aggregations_len..];
                let watermark = agg.watermark;
                // TODO: Should assert that there is only one result.
                if watermark > 0 {
                    ...
                    let data_end = data_start + len;
                    let res: IntermediateAggregationResults =
                        postcard::from_bytes(&buffer[data_start..data_end]).ok()?;
                    return Some(res);
                } else {
                    return None;
                }
