...
pub struct MutableSegmentEntry {
    pub ctid: u64,
    pub deleted: bool,
}
...
/// either A. FrozenTransactionId, or B. GetCurrentTransactionId(). We treat all such tags as
/// representing an `Immutable` segment in `impl From<u32> for Self`.
///
...
impl From<u32> for SegmentMetaEntryTag {
    fn from(value: u32) -> Self {
        ...
        if value == SegmentMetaEntryTag::Immutable as u32
            || pg_sys::TransactionId::from(value) >= pg_sys::FirstNormalTransactionId
        {
...
impl SegmentMetaEntry {
    ...
    pub fn mutable_add_items(
        ...
        match &mut self.content {
            SegmentMetaEntryContent::Mutable(content) if !content.frozen => {
                ...
                if new_max_doc >= gucs::mutable_segments_size() {
                    content.frozen = true;
                }
            }
    ...
    pub fn mutable_delete_items(
        ...
        // TODO: This is N^2, but for small N.
        for ctid in ctids {
            unsafe {
                entries
                    .update_item(
                        |entry| entry.ctid == ctid,
                        |entry| {
                            if !entry.deleted {
                                entry.deleted = true;
                                deleted += 1;
                    ...
                    .expect("Failed to update item in `mutable_delete_items`.");
            }
        }
    ...
    pub fn as_tantivy(&self) -> tantivy::index::InnerSegmentMeta {
        let deletes = match self.content {
            ...
            SegmentMetaEntryContent::Mutable(content) if content.num_deleted_docs > 0 => {
                Some(tantivy::index::DeleteMeta {
    ...
    pub fn replace_deletes(&mut self, delete: DeleteEntry) -> Option<Self> {
        ...
        match &mut self.content {
            ...
            SegmentMetaEntryContent::Mutable(_) => {
                // FIXME: See `delete.rs`: this codepath should no longer be reachable, but we
                // should stop using `save_new_metas` to replace deletes, and do it more directly
    ...
    pub fn byte_size(&self) -> u64 {
        let content = match self.content {
            SegmentMetaEntryContent::Immutable(ref content) => content,
            SegmentMetaEntryContent::Mutable(_) => {
                ...
                let num_docs: u64 = self.num_docs().try_into().unwrap();
                return 1000 * num_docs;
            }
        };
