...
impl From<u32> for SegmentMetaEntryTag {
    fn from(value: u32) -> Self {
        let value: pg_sys::TransactionId = value.into();
        if value < pg_sys::FirstNormalTransactionId && value != pg_sys::FrozenTransactionId {
            panic!("Expected a valid transaction id, or `FrozenTransactionId`, got: {value}");
        }
