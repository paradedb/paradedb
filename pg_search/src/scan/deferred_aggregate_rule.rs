            ...
            if agg_children.len() == 1 {
                if let Some(lookup) = agg_children[0].downcast_ref::<TantivyLookupExec>() {
                    ...
                    let resolved_input = if let Some(resolved) = resolved_input {
                        ...
                        // Remap deferred_fields to lookup_child's schema
                        let mut remapped_deferred_fields = Vec::new();
                        let child_schema = lookup_child.schema();
                        for df in &deferred_fields {
                            if let Some((idx, _)) = child_schema.fields().iter().enumerate().find(|(_, f)| f.name() == &df.display_name) {
                                let mut new_df = df.clone();
                                new_df.col_idx = idx;
                                remapped_deferred_fields.push(new_df);
                            } else {
                                return Err(datafusion::error::DataFusionError::Internal(format!(
                                    "DeferredAggregateRule: could not locate column for deferred field '{}' in child schema.",
                                    df.display_name
                                )));
                            }
                        }
