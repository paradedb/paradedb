...
use datafusion::common::{internal_err, JoinType, Result};
...
use crate::scan::plan_walk::{transform_up_with_children, visit_with_path, PathStep};
...
fn collect_decisions(node: &Arc<dyn ExecutionPlan>, bound: Bound, ctx: &mut Context) -> Result<()> {
    ...
    let topk = node
        ...
        .filter(|sort| sort.fetch().is_some());
    let below = if is_streaming_limit(node) {
        Bound::Limit
    } else if let Some(sort) = topk {
...
fn segmented_topk_takes(order: &LexOrdering, decode: &TantivyDecodeExec) -> bool {
    ...
    for sort in order.iter() {
        ...
        let Some(idx) = resolve_physical_index(col, &input_schema) else {
            continue;
        };
        if let Some(field) = decode.deferred_fields().iter().find(|f| f.col_idx == idx) {
...
fn collect_scans(
    ...
    visit_with_path(node, &mut |node, path| {
        ...
        let Some(scan) = node.downcast_ref::<PgSearchScanPlan>() else {
            return;
        };
        if let Some(field) = scan.deferred_fields().first() {
            if wanted.contains(&field.heap_rti) {
