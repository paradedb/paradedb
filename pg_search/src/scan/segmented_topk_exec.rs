...
pub struct SegmentedTopKExec {
    ...
    /// keeps one helper per alias (shared with the deferred lookup nodes).
...
impl SegmentedTopKState {
    ...
    fn materialize_deferred_ordinal(
        ...
        use datafusion::common::ScalarValue;
        match SegmentedTopKExec::ffhelper_for(&self.ffhelpers, deferred)?
    ...
    fn emit_final_topk(&mut self) -> Result<Option<RecordBatch>> {
        ...
        if !self.visibility_entries.is_empty() && !candidates.is_empty() {
            let row_keys: Vec<(usize, usize)> =
                candidates.iter().map(|(bi, ri, _, _)| (*bi, *ri)).collect();
