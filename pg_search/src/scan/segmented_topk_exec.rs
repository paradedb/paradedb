...
//! Per-segment streaming Top K with global dynamic filter.
//!
...
//!
//! A **global heap** of K entries is maintained across all completed segments.
//! When full, the worst entry's deferred ordinals are converted back to
//! materialized string literals via `FFHelper::ord_to_str` and published as a
//! global threshold. The scanner's `pre_filter::try_rewrite_binary` translates
//! these string literals to per-segment ordinal bounds automatically, pruning
//! rows at the scan level before they ever reach SegmentedTopKExec.
//!
...
impl ExecutionPlan for SegmentedTopKExec {
    ...
    fn execute(
        ...
        let stream_gen = async_stream::try_stream! {
            ...
            if let Some(last_seg) = state.current_segment {
                for fb in state.flush_segment(last_seg)? {
                    if fb.num_rows() > 0 {
                        state.rows_output.add(fb.num_rows());
                        yield fb;
                    }
