    ...
    pub fn try_enable_late_materialization(
        ...
        for wff in self.fields.iter_mut() {
            match wff {
                ...
                WhichFastField::Ctid => {
                    // Unconditional: this method is only called from the joinscan path,
                    // where visibility is always deferred to VisibilityFilterExec.
                    // Unlike Named fields (gated on is_string_or_bytes), ctid deferral
                    // is required for correctness — the scanner must emit packed
                    // DocAddresses so the join operates in a consistent identifier domain.
                    *wff = WhichFastField::DeferredCtid(crate::scan::ctid_column_name(rti));
                }
                _ => {}
            }
        }
