...
impl fmt::Display for EnvError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            EnvError::PosthogApiKey => write!(f, "Failed to retrieve POSTHOG_API_KEY from environment variables, not sending pg_search telemetry!"),
            EnvError::PosthogHost => write!(f, "Failed to retrieve POSTHOG_HOST from environment variables, not sending pg_search telemetry!"),
            EnvError::CommitSha => write!(f, "Failed to retrieve COMMIT_SHA from environment variables, sending pg_search telemetry without commit_sha!"),
            EnvError::ParsingError => write!(f, "Telemetry for pg_search disabled!"),
        }
    }
}
