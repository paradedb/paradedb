...
-- Force the window aggregate onto the parallel path. The
-- `max_window_aggregate_response_bytes` size guard tested below is parallel-only
...
-- erroring. The #3055 row cap (`min_rows_per_worker`, default 300K) would otherwise
-- serialize this small (50K-row) fixture, so disable it here to exercise the guard.
