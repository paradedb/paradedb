...
WITH (text_fields='{"description": {}, "category": {"fast": true}, "brand": {"fast": true}}',
      numeric_fields='{"price": {"fast": true}}');
CREATE INDEX aog_reviews_idx ON aog_reviews
USING bm25 (id, product_id, stars)
WITH (numeric_fields='{"product_id": {"fast": true}, "stars": {"fast": true}}');
CREATE INDEX aog_notes_idx ON aog_notes
USING bm25 (id, product_id, note)
WITH (numeric_fields='{"product_id": {"fast": true}}', text_fields='{"note": {"fast": true}}');
