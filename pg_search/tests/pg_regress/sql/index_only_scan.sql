...
CREATE TABLE index_only_terms (term text, pattern text, min_id bigint);
INSERT INTO index_only_terms VALUES ('needle', '%missing%', 10000), ('missing', '%needle%', 0);
ANALYZE index_only_terms;
SET enable_hashjoin = off;
SET enable_mergejoin = off;
SET paradedb.enable_aggregate_custom_scan = off;

-- Parameterized index conditions and covered join filters preserve index-only scans.
DO $$
DECLARE
    plan json;
    query text;
BEGIN
    FOREACH query IN ARRAY ARRAY[
        ...
        IF plan #>> '{0,Plan,Node Type}' IS DISTINCT FROM 'Nested Loop'
            OR plan #>> '{0,Plan,Plans,1,Node Type}' IS DISTINCT FROM 'Index Only Scan'
            OR (plan #>> '{0,Plan,Plans,1,Heap Fetches}')::int IS DISTINCT FROM 0
            OR (plan #>> '{0,Plan,Plans,1,Actual Loops}')::numeric IS DISTINCT FROM 2
            OR position('t.term' IN plan #>> '{0,Plan,Plans,1,Index Cond}') = 0
        THEN
            RAISE EXCEPTION 'expected a parameterized index-only scan without heap fetches: %', plan;
        END IF;
    END LOOP;
END;
$$;
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION 'expected a heap scan for a nonreturnable join filter: %', plan;
    END IF;
END;
$$;
DO $$
...
-- Parallel index-only scans work with and without an eligible parallel heap path.
DO $$
DECLARE
    plan json;
    heap_threshold text;
BEGIN
    FOREACH heap_threshold IN ARRAY ARRAY['0', '1GB'] LOOP
        ...
        IF plan #>> '{0,Plan,Node Type}' IS DISTINCT FROM 'Gather'
            ...
            OR plan #>> '{0,Plan,Plans,0,Node Type}' IS DISTINCT FROM 'Index Only Scan'
            OR (plan #>> '{0,Plan,Plans,0,Parallel Aware}')::boolean IS DISTINCT FROM true
            OR (plan #>> '{0,Plan,Plans,0,Heap Fetches}')::int IS DISTINCT FROM 0
            OR (plan #>> '{0,Plan,Actual Rows}')::numeric IS DISTINCT FROM 20000
        THEN
            RAISE EXCEPTION 'expected a parallel index-only scan without heap fetches: %', plan;
        END IF;
    END LOOP;
END;
$$;
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION 'expected a serial index-only scan below the parallel threshold: %', plan;
    END IF;
END;
$$;
