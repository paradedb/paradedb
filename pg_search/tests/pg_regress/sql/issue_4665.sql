...
-- Inflate the per-segment cost so our score-DESC TopK cost comparison picks
-- parallel even for this small dataset; this keeps the test exercising the
-- parallel-worker-selection path that #4665 was specifically about.
...
-- ============================================================================
-- DEFAULT paradedb.per_segment_cost documentation
--
-- The earlier sections inflate per_segment_cost = 1000 to keep the test
-- exercising the parallel-worker-selection path that #4665 was originally
-- diagnosed against. This section drops back to the default and documents the
-- conservative #4664 gate: CUSTOM can resolve $1 at plan time and may use the
-- bounded score-DESC TopK cost branch, while GENERIC keeps $1 as a
-- PostgresExpression and uses general worker selection because the
-- planner cannot prove the runtime query will be Block-WAND-prunable. The
-- important #4665 invariant remains result parity, which the executions above
-- check in both plan modes.
-- ============================================================================
RESET paradedb.per_segment_cost;

SET plan_cache_mode = force_custom_plan;
PREPARE issue_4665_parity_custom(text) AS
SELECT id FROM issue_4665_test
WHERE content ||| $1
ORDER BY pdb.score(id) DESC
LIMIT 10;
...
EXPLAIN (FORMAT TEXT, COSTS OFF, TIMING OFF)
EXECUTE issue_4665_parity_custom('technology');

DEALLOCATE issue_4665_parity_custom;

SET plan_cache_mode = force_generic_plan;
PREPARE issue_4665_parity_generic(text) AS
SELECT id FROM issue_4665_test
WHERE content ||| $1
ORDER BY pdb.score(id) DESC
LIMIT 10;
...
EXPLAIN (FORMAT TEXT, COSTS OFF, TIMING OFF)
EXECUTE issue_4665_parity_generic('technology');
