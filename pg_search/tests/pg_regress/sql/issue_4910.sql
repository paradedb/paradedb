...
-- related tables via EXISTS and NOT EXISTS. In parallel mode, the planner
-- previously rejected the JoinScan candidate at two gates:
--   1. RIGHT_ANTI / RIGHT_SEMI / UNIQUE_* jointypes from the parallel-aware
--      side-swap variants were rejected as "unsupported".
--   2. The 3-way absorption walker bailed when cheapest_total_path was
--      wrapped in GatherPath / GatherMergePath / MaterialPath, so the third
--      relation stayed un-absorbed and is_limit_pushdown_safe failed.
--
...
-- model picks serial paths at small scale and the failing code paths
-- (RIGHT_ANTI decode, GatherPath wrapper peeling) are not exercised. With
-- the GUC, the same `Gather Merge -> Parallel Custom Scan (ParadeDB Join
-- Scan)` shape that the original 1M-row repro produced is reproduced at 1K
-- rows. Do NOT remove `debug_parallel_query` without scaling row counts up.
...
-- Join` over `Parallel Hash Semi Join` and emitted three planner warnings.
