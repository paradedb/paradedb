...
CREATE INDEX ON issue_join_pre_filter_users USING paradedb (id, reputation) WITH (sort_by='id ASC NULLS FIRST');
...
CREATE INDEX ON issue_join_pre_filter_posts USING paradedb (id, (title::pdb.unicode_words('columnar=true')), owner_user_id) WITH (sort_by='owner_user_id ASC NULLS FIRST');
