...
CREATE INDEX orders_bm25_idx ON orders USING paradedb (id, description, (customer_code::pdb.literal));
CREATE INDEX customers_bm25_idx ON customers USING paradedb ((customer_code::pdb.literal), name, email);
...
CREATE INDEX inventory_bm25_idx ON inventory USING paradedb (id, product_name, region_id, (warehouse_code::pdb.literal));
CREATE INDEX warehouses_bm25_idx ON warehouses USING paradedb (region_id, (warehouse_code::pdb.literal), name, description);
...
CREATE INDEX item_types_bm25_idx ON item_types USING paradedb (type_id, type_name, description);
...
CREATE INDEX docs_bm25_idx ON docs USING paradedb (id, title, content, (author_code::pdb.literal));
CREATE INDEX authors_bm25_idx ON authors USING paradedb ((author_code::pdb.literal), name, bio);
...
CREATE INDEX categories_nulls_bm25_idx ON categories_with_nulls USING paradedb (id, name, description);
...
WHERE (i.content ||| 'item' OR i.content ||| 'laptop' OR i.content ||| 'novel')
ORDER BY i.id
LIMIT 10;
...
-- UUID columns are columnar by default
CREATE INDEX uuid_orders_bm25_idx ON uuid_orders USING paradedb (id, description, (customer_id::pdb.literal));
-- uuid_customers.id is fast by default
CREATE INDEX uuid_customers_bm25_idx ON uuid_customers USING paradedb (id, name, email);
...
-- numeric_accounts.account_num is fast by default
