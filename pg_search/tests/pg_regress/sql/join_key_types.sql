...
WITH (text_fields = '{"customer_code": {"fast": true, "tokenizer": {"type": "keyword"}}}');
...
WITH (numeric_fields = '{"region_id": {"fast": true}}',
...
WITH (numeric_fields = '{"type_id": {"fast": true}}');
...
WITH (text_fields = '{"author_code": {"fast": true, "tokenizer": {"type": "keyword"}}}');
...
WITH (numeric_fields = '{"line_num": {"fast": true}}');
CREATE INDEX order_items_bm25_idx ON order_items USING paradedb (id, notes, order_id, line_num)
WITH (numeric_fields = '{"order_id": {"fast": true}, "line_num": {"fast": true}}');
...
-- UUID columns are fast by default and also support explicit text_fields config
CREATE INDEX uuid_orders_bm25_idx ON uuid_orders USING paradedb (id, description, customer_id)
WITH (text_fields = '{"customer_id": {"fast": true, "tokenizer": {"type": "keyword"}}}');
...
WITH (numeric_fields = '{"account_num": {"fast": true}}');
