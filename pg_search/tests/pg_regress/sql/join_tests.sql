...
-- Create BM25 indexes
CREATE INDEX authors_bm25_idx ON authors USING paradedb (id, name, bio, country) WITH (key_field = 'id');
CREATE INDEX books_bm25_idx ON books USING paradedb (id, title, content) WITH (key_field = 'id');
CREATE INDEX reviews_bm25_idx ON reviews USING paradedb (id, content) WITH (key_field = 'id');
CREATE INDEX categories_bm25_idx ON categories USING paradedb (id, name, description) WITH (key_field = 'id');
CREATE INDEX publishers_bm25_idx ON publishers USING paradedb (id, name, description) WITH (key_field = 'id');
