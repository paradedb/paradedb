-- Test for JSONB ? (exists) operator pushdown
...
CREATE INDEX idx_jsonb_exists_test ON jsonb_exists_test USING bm25 (id, description, data)
...
SELECT * FROM jsonb_exists_test WHERE data ? 'first_name' ORDER BY id;
...
SELECT * FROM jsonb_exists_test WHERE data ? 'first_name' AND data ? 'last_name' ORDER BY id;

-- Test 4: Verify equivalence with paradedb.exists() - should return same results
SELECT * FROM jsonb_exists_test WHERE id @@@ paradedb.exists('data.first_name') ORDER BY id;

-- Test 5: JSONB ? with nested path using -> operator
-- data->'address' ? 'city' checks if 'city' key exists in data.address
SELECT * FROM jsonb_exists_test WHERE data->'address' ? 'city' ORDER BY id;

-- Test 6: Verify equivalence with paradedb.exists() for nested path
SELECT * FROM jsonb_exists_test WHERE id @@@ paradedb.exists('data.address.city') ORDER BY id;

-- Test 7: NOT EXISTS using JSONB ? operator
SELECT * FROM jsonb_exists_test WHERE NOT (data ? 'first_name') ORDER BY id;
