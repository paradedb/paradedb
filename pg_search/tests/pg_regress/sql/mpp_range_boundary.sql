...
WITH (target_segment_count=8, partition_by='id', text_fields='{"display_name":{"tokenizer":{"type":"keyword"},"fast":true}}', numeric_fields='{"reputation":{"fast":true}}');
CREATE INDEX rb_posts_idx ON rb_posts USING paradedb (id, owner_user_id, title)
WITH (target_segment_count=8, partition_by='id,owner_user_id', text_fields='{"title":{"fast":true}}', numeric_fields='{"owner_user_id":{"fast":true}}');
CREATE INDEX rb_comments_idx ON rb_comments USING paradedb (id, post_id, text, score)
WITH (target_segment_count=8, partition_by='post_id', text_fields='{"text":{"fast":true}}', numeric_fields='{"post_id":{"fast":true},"score":{"fast":true}}');
