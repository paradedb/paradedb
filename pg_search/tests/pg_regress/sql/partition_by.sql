...
CREATE INDEX partition_by_test_idx ON partition_by_test
    ...
    WITH (partition_by='tenant_id, created_at');
DROP INDEX partition_by_test_idx;
...
\echo 'Test 1.4: partition_by with ctid'
CREATE INDEX partition_by_test_idx ON partition_by_test
    USING paradedb (id, name, tenant_id)
    WITH (partition_by='ctid');
SELECT id, name FROM partition_by_test WHERE name ||| 'Alice';
DROP INDEX partition_by_test_idx;

\echo 'Test 1.5: partition_by with multiple fields including ctid'
CREATE INDEX partition_by_test_idx ON partition_by_test
    USING paradedb (id, name, tenant_id)
