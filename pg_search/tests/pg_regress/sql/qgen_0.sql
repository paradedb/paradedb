...
SET seed TO 0.2929695368068037;
-- PARADEDB_QGEN_SEED: 17532156400539643165
-- qgen bulk inserts: 8
...
INSERT into users (uuid, name, color, age, quantity, price, small_numeric, int_numeric, high_scale, big_numeric, rating, category, literal_normalized, metadata, tags) SELECT rpad(lpad((random() * 2147483647)::integer::text, 10, '0'), 32, '0')::uuid,
      ...
      (CASE (floor(random() * 5) + 1)::int WHEN 1 THEN ARRAY['alpha', 'beta']::text[] WHEN 2 THEN ARRAY['gamma']::text[] WHEN 3 THEN ARRAY['delta', 'epsilon', 'zeta']::text[] WHEN 4 THEN NULL ELSE ARRAY[]::text[] END) FROM generate_series(1, 1);
...
INSERT into users (uuid, name, color, age, quantity, price, small_numeric, int_numeric, high_scale, big_numeric, rating, category, literal_normalized, metadata, tags) SELECT rpad(lpad((random() * 2147483647)::integer::text, 10, '0'), 32, '0')::uuid,
      ...
      (CASE (floor(random() * 5) + 1)::int WHEN 1 THEN ARRAY['alpha', 'beta']::text[] WHEN 2 THEN ARRAY['gamma']::text[] WHEN 3 THEN ARRAY['delta', 'epsilon', 'zeta']::text[] WHEN 4 THEN NULL ELSE ARRAY[]::text[] END) FROM generate_series(1, 2);
...
-- PostgreSQL query:
EXPLAIN
SELECT DISTINCT users.id, users.name, products.id, orders.id, users.quantity IS NOT NULL, users.quantity FROM users JOIN products ON users.id = products.id CROSS JOIN orders WHERE NOT (orders.color  =  'blue') ORDER BY users.quantity IS NOT NULL, users.quantity, users.id, products.id, orders.id LIMIT 20;
SELECT DISTINCT users.id, users.name, products.id, orders.id, users.quantity IS NOT NULL, users.quantity FROM users JOIN products ON users.id = products.id CROSS JOIN orders WHERE NOT (orders.color  =  'blue') ORDER BY users.quantity IS NOT NULL, users.quantity, users.id, products.id, orders.id LIMIT 20;
--
