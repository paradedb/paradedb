...
-- paradedb.score() and paradedb.snippet() with a SELECT subquery in WHERE clause
...
-- Test 1: paradedb.score() with (SELECT true) should use ParadeDB Base Scan
...
-- Test 2: paradedb.snippet() with (SELECT true) should use ParadeDB Base Scan
EXPLAIN (FORMAT TEXT, COSTS OFF, TIMING OFF)
SELECT id, paradedb.snippet(description) AS snippet
FROM animals
WHERE description ||| 'dog' AND (SELECT true)
LIMIT 1;
...
-- Test 4: paradedb.score() with a function subquery (RLS policy pattern)
...
-- Test 5: paradedb.score() with a table lookup subquery
