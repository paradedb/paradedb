...
FROM paradedb.index_info('radii_probe_idx');

-- One radius row per cluster row: the radii SRF mirrors ivf_cluster_sizes'
-- (segno, field, cluster_ord) iteration exactly.
SELECT (SELECT count(*) FROM paradedb.ivf_cluster_radii('radii_probe_idx'))
     = (SELECT count(*) FROM paradedb.ivf_cluster_sizes('radii_probe_idx'))
...
FROM paradedb.ivf_cluster_radii('radii_probe_idx');
...
-- IVF-segments-only, like ivf_cluster_sizes.
...
SELECT count(*) AS flat_rows FROM paradedb.ivf_cluster_radii('radii_flat_idx');
