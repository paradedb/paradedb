...
# Test generic xlog record work for shnsw index replication.
use strict;
use warnings;
use PostgresNode;
use TestLib;
...
# Create shnsw index on primary
$node_primary->safe_psql("postgres", "CREATE EXTENSION svector;");
$node_primary->safe_psql("postgres", "CREATE TABLE tst (i int4, v svector($dim));");
$node_primary->safe_psql("postgres",
	"INSERT INTO tst SELECT i % 10, ARRAY[$array_sql] FROM generate_series(1, 1000) i;"
);
$node_primary->safe_psql("postgres", "CREATE INDEX ON tst USING shnsw (v svectorl2_ops);");
