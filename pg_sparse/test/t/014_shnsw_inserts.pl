...
$node->safe_psql("postgres", "CREATE TABLE tst (v svector($dim));");
$node->safe_psql("postgres", "CREATE INDEX ON tst USING shnsw (v svectorl2_ops);");
...
for my $i (1 .. 20)
{
	$node->pgbench(
		...
		"concurrent INSERTs",
		{
			"014_shnsw_inserts_$i" => "INSERT INTO tst VALUES (ARRAY[$array_sql]);"
		}
	);
...
$node->pgbench(
	...
	"concurrent INSERTs",
	{
		"014_shnsw_inserts" => "INSERT INTO tst SELECT ARRAY[$array_sql] FROM generate_series(1, 10) i;"
	}
);

my $count = $node->safe_psql("postgres", qq(
	SET enable_seqscan = off;
	SET shnsw.ef_search = 1000;
	SELECT COUNT(*) FROM (SELECT v FROM tst ORDER BY v <-> (SELECT v FROM tst LIMIT 1)) t;
));
