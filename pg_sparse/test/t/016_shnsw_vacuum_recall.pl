...
$node->safe_psql("postgres", "CREATE TABLE tst (i int4, v svector(3));");
...
# Add index
$node->safe_psql("postgres", "CREATE INDEX ON tst USING shnsw (v svectorl2_ops) WITH (m = 4, ef_construction = 8);");
