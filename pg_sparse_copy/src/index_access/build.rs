...
use crate::index_access::utils::{
    categorize_tupdesc, create_sparse_index, lookup_index_tupdesc, row_to_json,
};
...
impl<'a> BuildState<'a> {
    fn new(sparse_index: &'a mut SparseIndex, writer: &'a mut SingleSegmentIndexWriter) -> Self {
        BuildState {
            sparse_index,
            writer,
            count: 0,
...
pub extern "C" fn ambuild(
    ...
    // rdopts are passed on to create_sparse_index
    ...
    let mut sparse_index = create_sparse_index(
        index_name.clone(),
        format!("{}.{}", schema_name, table_name),
        rdopts,
    );
    let tantivy_index = sparse_index.copy_tantivy_index();
...
fn do_heap_scan<'a>(
    ...
    let mut state = BuildState::new(sparse_index, writer);
...
unsafe extern "C" fn build_callback_internal(
    ...
    state.sparse_index.insert(state.writer, ctid, builder);
