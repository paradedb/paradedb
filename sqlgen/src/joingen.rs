...
/// A single step in the chain of joins.
/// If `join_type == Cross`, the `on_*` fields are `None`.
#[derive(Clone, Debug)]
struct JoinStep {
    join_type: JoinType,
    table_idx: usize,
...
/// A complete “FROM … JOIN … JOIN …” expression.
#[derive(Clone, Debug)]
struct JoinExpr {
    initial_table: usize,
    steps: Vec<JoinStep>,
...
impl JoinExpr {
    /// Returns the list of table‐indices used so far.
