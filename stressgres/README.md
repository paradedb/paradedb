...
```bash
cargo run -- ui suites/single-node-planner-paths.toml
```

- Run headless mode with logging:

```bash
cargo run -- headless suites/single-node-planner-paths.toml --runtime=300000 --log-file=logs/test.log
```

- Run headless mode tolerating transient database faults (e.g. under Antithesis)

```bash
cargo run -- headless suites/single-node-planner-paths.toml --runtime=300000 --reconnect-grace=30000
```

- Run a suite against a throwaway Postgres cluster built from a given `pg_config`:

```bash
cargo run -- auto /path/to/pg_config suites/single-node-planner-paths.toml /tmp/stressgres-data --runtime 300000
```

Suites are TOML files in `suites/`. Each suite describes a planner workload or PostgreSQL topology exercised by Stressgres.
...
```bash
docker run --rm paradedb/stressgres:latest cargo run -- headless suites/single-node-planner-paths.toml
```
