...
/// #6023: a self-join that sorts by a late-materialized column on *both* aliases
/// must not share one `FFHelper`. Alias `a` projects extra fields (`message`,
/// `ref_id`) so `name`'s `ff_index` differs from alias `b`.
#[rstest]
fn joinscan_self_join_asymmetric_deferred_sort_matches_fallback(
    mut conn: PgConnection,
) -> Result<(), sqlx::Error> {
    r#"
    SET paradedb.enable_custom_scan = on;
    ...
    let query = r#"
        SELECT a.name, b.name FROM sj a JOIN sj b ON a.ref_id = b.id
        WHERE a.message @@@ 'beer'
        ORDER BY a.name, b.name LIMIT 5
    "#;
    ...
    assert_eq!(
        fallback_rows,
        vec![
            ("name 00001".into(), "name 00002".into()),
            ("name 00004".into(), "name 00005".into()),
            ("name 00005".into(), "name 00006".into()),
            ("name 00008".into(), "name 00009".into()),
            ("name 00009".into(), "name 00010".into()),
        ]
    );
