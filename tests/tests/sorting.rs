...
fn field_sort_fixture(conn: &mut PgConnection) -> Value {
    ...
    let (plan, ) = r#"EXPLAIN (ANALYZE, FORMAT JSON) SELECT * FROM paradedb.bm25_search WHERE description @@@ 'keyboard OR shoes' ORDER BY lower(category) COLLATE "C" LIMIT 5"#.fetch_one::<(Value,)>(conn);
    eprintln!("{plan:#?}");
    plan
}
...
fn sort_by_raw(mut conn: PgConnection) {
    ...
    let (plan, ) = r#"EXPLAIN (ANALYZE, FORMAT JSON) SELECT * FROM paradedb.bm25_search WHERE description @@@ 'keyboard OR shoes' ORDER BY category COLLATE "C" LIMIT 5"#.fetch_one::<(Value,)>(&mut conn);
    eprintln!("{plan:#?}");
    let plan = plan
        .pointer("/0/Plan/Plans/0")
        .unwrap()
